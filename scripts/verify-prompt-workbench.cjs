...
const qaApp = { id: '11111111-2222-4333-8444-555555555555', label: 'Codex' };
...
app.whenReady().then(async () => {
  ...
  channels.forEach(channel => ipcMain.handle(channel, (_e, value) => {
    ...
    if (channel === 'desktop:list-registered-applications') return [qaApp];
    if (channel === 'desktop:choose-application') return { cancelled: true };
  ...
  const orderBefore = controller.snapshot().rows.slice(0, 2);
  record('first prompt cannot move past the top boundary', await run(`document.querySelector(${JSON.stringify(`[aria-label="上移 ${orderBefore[0].title}"]`)}).disabled`));
  await run(`document.querySelector(${JSON.stringify(`[aria-label="上移 ${orderBefore[1].title}"]`)}).click()`); await pause(180);
  record('up button swaps only adjacent prompt order and keeps the moved prompt selected', controller.snapshot().rows[0].id === orderBefore[1].id && controller.snapshot().rows[1].id === orderBefore[0].id && controller.snapshot().selectedId === orderBefore[1].id);
  await controller.command({ type: 'view', query: '重启' }); await pause(120);
  record('ordering controls stay out of search results', await run(`!document.querySelector('.prompt-order-tools')`));
  await controller.command({ type: 'view', query: '' }); await pause(120);
  ...
  await run(`(() => { const select=document.querySelector('[aria-label="场景按键动作"]'); Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set.call(select,'save'); select.dispatchEvent(new Event('change',{bubbles:true})); })()`); await pause(120);
  await clickText('保存当前按键');
  record('common action choice saves shortcut and label without manual typing', store.data.scenes.find(s=>s.id==='scene-video').bindings[5].value==='Ctrl+S' && store.data.scenes.find(s=>s.id==='scene-video').bindings[5].label==='保存');
  ...
  await clickText('取消');
  await run(`document.querySelector('[data-key="5"]').click()`); await pause(180);
  await run(`(() => { const select=document.querySelector('[aria-label="场景按键动作"]'); Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set.call(select,'hotkey'); select.dispatchEvent(new Event('change',{bubbles:true})); })()`); await pause(120);
  ...
  await run(`document.querySelector('[data-key="6"]').click()`); await pause(120);
  await run(`(() => { const select=document.querySelector('[aria-label="场景按键动作"]'); Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set.call(select,'app'); select.dispatchEvent(new Event('change',{bubbles:true})); })()`); await pause(120);
  record('open application uses registered whitelist picker with no command field', await run(`document.querySelector('[aria-label="场景打开应用"]').textContent.includes('Codex') && !document.querySelector('[aria-label="场景快捷键"]')`));
  await shot('keymap-open-app-1440');
  await run(`(() => { const select=document.querySelector('[aria-label="场景打开应用"]'); Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype,'value').set.call(select,${JSON.stringify(qaApp.id)}); select.dispatchEvent(new Event('change',{bubbles:true})); })()`); await pause(120);
  await clickText('保存当前按键');
  record('registered application action persists by opaque ID only', store.data.scenes.find(s=>s.id==='coding').bindings[6].type==='app' && store.data.scenes.find(s=>s.id==='coding').bindings[6].appActionId===qaApp.id && !JSON.stringify(store.data.scenes.find(s=>s.id==='coding').bindings[6]).includes('.exe'));
