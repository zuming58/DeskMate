...
const now = Date.now();
let reminderItems = [{ id: '00000000-0000-4000-8000-000000000001', title: '准备展会材料', important: true, remindAt: now + 3600000, eventAt: now + 7200000, status: 'pending', createdAt: now, updatedAt: now, notifiedAt: null }];
...
const reminderSnapshot = () => ({ ready: true, reason: '', revision: reminderItems.length, activeCount: reminderItems.length, todayCount: reminderItems.length, items: reminderItems });
const overview = () => createWorkbenchOverview({ memoryStore: { dashboardSummary: memory }, promptStore, reminderStore: { dashboardSnapshot: reminderSnapshot },
...
app.whenReady().then(async () => {
  ...
  channels.forEach(channel => ipcMain.handle(channel, (_event, payload) => {
    calls.push(channel);
    if (channel === 'local-backup:handover') return null;
  ...
  record('failed reminder controls do not overflow compact window', await run(`document.documentElement.scrollWidth<=innerWidth`));
  await run(`document.querySelector('.wb-reminders-card').scrollIntoView({block:'center'})`); await pause(200);
  await shot('reminder-failed-960-synthetic');
  ...
  await click('AI 陪伴');
  await waitFor(() => run(`document.querySelector('.companion-portrait img')?.complete && document.querySelector('.companion-portrait img')?.naturalWidth>0 && document.querySelectorAll('.companion-portrait video').length===2`), 'companion portrait loaded');
  record('companion page uses one poster and two bounded video surfaces', await run(`document.querySelectorAll('.companion-portrait').length===1 && document.querySelectorAll('.companion-portrait video').length===2 && document.querySelectorAll('.companion-face--soft').length===1`));
  await shot('companion-1440-synthetic');
  ...
  window.setContentSize(1440,1024); await load();
  record('high-priority connections and reminders are visible at 1440x1024', await run(`document.querySelector('.wb-connections-card').getBoundingClientRect().top<innerHeight && document.querySelector('.wb-reminders-card').getBoundingClientRect().top<innerHeight`));
...
}).catch(async error => { let page=''; try { page=await window?.webContents?.executeJavaScript(`document.documentElement.outerHTML.slice(0,4000)`); fs.writeFileSync(path.join(output,'failure.png'),(await window.webContents.capturePage()).toPNG()); } catch {} fs.writeFileSync(path.join(output,'failure.json'),JSON.stringify({error:String(error?.stack||error),assertions,errors,page},null,2)); console.error(error); console.error(JSON.stringify({ output, assertions, errors })); app.exit(1); });
