...
export function DashboardPage({ navigate }) {
  ...
  const xiaozhiDisabled = board.xiaozhiHardware?.enabled !== true;
  ...
  const metrics = [
    ...
    { label: '长期记忆', value: number(memory?.longTermMemories), unit: '条', icon: IconBrain, tone: 'green', caption: `${number(memory?.curation?.questions)} 组需核对 · ${number(memory?.curation?.pending)} 条待自动整理`, title: '包含有依据的自动记忆和你确认的内容，均可纠正或删除。' },
  ];
  return <div className="page page--dashboard wb-overview">
    ...
    <Card className="wb-connections-card"><PanelHeading icon={IconLink} title="连接与服务" link="设备与诊断" onClick={() => navigate('settings/diagnostics')} /><div className="wb-connections">
      ...
      <Connection icon={IconDatabase} title="KnowledgeOS" label={!knowledge ? '待读取' : !(knowledge.readEnabled || knowledge.syncEnabled) ? '已关闭' : knowledge.configured ? '已配置' : '待配置'} detail={!(knowledge?.readEnabled || knowledge?.syncEnabled) ? '本地保存 · 无需外部软件' : knowledge?.configured ? `检索${knowledge.readEnabled ? '开启' : '关闭'} · 同步${knowledge.syncEnabled ? '开启' : '关闭'}` : '连接你自己的 KnowledgeOS'} tone={knowledge?.configured && (knowledge.readEnabled || knowledge.syncEnabled) ? 'demo' : 'neutral'} onClick={() => navigate('memory')} />
    </div></Card>
    ...
    <div className="wb-main-grid">
      ...
      <Card className="wb-memory"><PanelHeading icon={IconBrain} title="记忆正在积累" link="记忆管理" onClick={() => navigate('memory')} /><div className="wb-memory-totals"><div><strong>{number(memory?.completedJournals)}</strong><span>份日终总结</span></div><div><strong>{number(memory?.pendingSync)}</strong><span>份等待同步</span></div><StatusBadge tone={policy?.running ? 'demo' : 'neutral'}>{workbenchSchedule(policy, memory?.latestJournal)}</StatusBadge></div>
        ...
        <div className="wb-panel-footer"><span>{memory?.curation?.questions ? `${number(memory.curation.questions)} 组有疑问的内容需核对` : '记忆自动整理，疑问才需核对'}</span><button className="wb-text-link" onClick={() => navigate('memory')}>查看与整理<IconArrowUpRight size={15} /></button></div>
      </Card>
