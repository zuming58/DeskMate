...
export function AgentsPage({ notify, embedded = false }) {
  ...
  return (
    <div className={embedded ? "companion-embedded" : "page"}>
      ...
      <Card><SectionTitle index="02A" title="Codex 近期任务简报" description="可选 codex-task-brief-v1 只接收任务标签、粗状态和不超过 80 字的里程碑；不读取提示词、回复、工具参数、路径或窗口标题。" /><SettingRow title="主动语音播报" description="任务开始、等待、完成或报错时由豆包语音提醒；关闭后仍保留状态，可随时询问。"><Toggle checked={codexAnnouncementsEnabled} onChange={async (enabled) => { const result = await voiceAdapters.desktop.setCodexTaskBriefAnnouncements(enabled); if (!result?.ok) return notify(`播报设置未保存：${result?.reason || "unknown-error"}`); patch({ runtime: { ...state.runtime, codexTasks: { ...state.runtime.codexTasks, ...result } } }); notify(enabled ? "已开启 Codex 主动语音播报" : "已关闭 Codex 主动语音播报"); }} /></SettingRow>{recentCodexTasks.length ? <div className="companion-info-list">{recentCodexTasks.map((task) => <div key={`${task.taskLabel}-${task.sequence}`}><span><small>{task.state} · sequence {task.sequence}</small><strong>{task.taskLabel}</strong>{task.milestone && <small>{task.milestone}</small>}</span></div>)}</div> : <Notice tone="info" title="等待可选简报报告器">现有 codex-hook-v1 粗状态继续有效；尚未收到细粒度任务简报。</Notice>}</Card>
