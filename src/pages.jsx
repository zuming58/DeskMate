...
export function CompanionPage({ notify, navigate }) {
  ...
  const toggleSession = async () => {
    try {
      const result = sessionActive ? await globalThis.desktopBridge?.stopCompanionConversation?.() : await globalThis.desktopBridge?.startCompanionConversation?.({ microphoneSource: preferredCompanionSource, microphoneId: state.settings.microphoneId || "" });
    ...
    <div className="page page--companion">
      ...
      {section === "overview" && <>
        ...
        <Card className="companion-stage">
          ...
          <div className="button-row companion-dialogue-actions"><Button icon={sessionActive ? PlayerPause : MessageCircle} variant="primary" className="companion-dialogue-button" disabled={conversation.state === "stopping"} onClick={toggleSession}>{conversation.state === "stopping" ? "正在结束…" : sessionActive ? "结束陪伴对话" : conversation.state === "error" ? "重新开始陪伴对话" : "开始陪伴对话"}</Button>{sessionActive && ["thinking", "speaking", "completed"].includes(conversation.state) && <Button icon={PlayerPause} variant="ghost" onClick={interruptResponse}>打断回答并继续听</Button>}</div>
