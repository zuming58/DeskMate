...
export function MotionPage({ notify, embedded = false }) {
  ...
  useEffect(() => {
    ...
    void readWithRetry();
    voiceAdapters.desktop.listChoreographies().then((value) => { if (active) setDefaultDanceName(value?.defaultDanceName || ""); }).catch(() => {});
    ...
    <div className={embedded ? "companion-embedded" : "page"}>
      ...
      <div className="motion-grid">
        ...
        <Card className="motion-preset-card">
          ...
          <p className="motion-default-summary"><strong>默认</strong>关注、寻找 1 次；点头、跳舞 2 次。{defaultDanceName ? ` 当前“跳舞”使用自定义动作：${defaultDanceName}。` : " 当前“跳舞”使用内置动作。"}</p>
...
export function SettingsPage({ notify, initialSection = "" }) {
  ...
  const [motionSettings, setMotionSettingsState] = useState({ intensity: "standard", tempo: "standard" });
    ...
    <div className="page">
      ...
      <div className="settings-layout">
        ...
        {section === "connections" ? <div className="settings-panel settings-panel--connections"><ConnectionsPage notify={notify} embedded /></div> : <Card className="settings-panel">
          ...
          {section === "motion" && <><SectionTitle index="03" title="动作设置" description="统一调节关注、点头、寻找、跳舞和自定义舞蹈" /><SettingRow title="动作强度" description="调节实体动作幅度；“明显”仍严格限制在已经验收的 Stage 2 安全范围内"><Segmented value={motionSettings.intensity} onChange={(value) => { void saveMotionSettings({ intensity: value }); }} options={[{ value: "gentle", label: "柔和" }, { value: "standard", label: "标准" }, { value: "vivid", label: "明显" }]} /></SettingRow><SettingRow title="动作速度" description="调节每拍停留时间；舵机真实移动速度仍由小智固件安全限速"><Segmented value={motionSettings.tempo} onChange={(value) => { void saveMotionSettings({ tempo: value }); }} options={[{ value: "relaxed", label: "舒缓" }, { value: "standard", label: "标准" }, { value: "quick", label: "利落" }]} /></SettingRow><Notice tone="info" title="作用范围">设置从下一次实体动作开始生效，不改变软件预览速度，也不开放任意角度、PWM、GPIO 或舵机脉宽。</Notice></>}
