      ...
      <div className="prompt-columns">
        <div className="prompt-library">
          ...
          <div className="prompt-rows" role="list" aria-label="提示词列表">
            ...
            {d.rows.map((p, index) => { const Icon = sceneIcon(p.primarySceneId); return <div key={p.id} role="listitem" className={`prompt-row ${p.id === d.selectedId ? 'selected' : ''}`} data-selected={p.id === d.selectedId}>
              ...
              <div className="prompt-row-tools">{d.view.filter === 'trash' ? <button aria-label={`恢复 ${p.title}`} title="恢复" onClick={() => command({ type: 'restore', id: p.id, revision: d.revision })}><IconArrowBackUp size={18} /></button> : <>{canReorder && <span className="prompt-order-tools"><button aria-label={`上移 ${p.title}`} title="上移" disabled={index === 0} onClick={() => command({ type: 'reorder', id: p.id, direction: -1, revision: d.revision })}><IconArrowUp size={17} /></button><button aria-label={`下移 ${p.title}`} title="下移" disabled={index === d.rows.length - 1} onClick={() => command({ type: 'reorder', id: p.id, direction: 1, revision: d.revision })}><IconArrowDown size={17} /></button></span>}<button aria-label={`收藏 ${p.title}`} aria-pressed={d.favorites.includes(p.id)} title="收藏" onClick={() => command({ type: 'favorite', id: p.id, revision: d.revision })}><IconStar size={18} fill={d.favorites.includes(p.id) ? 'currentColor' : 'none'} /></button><button aria-label={`编辑 ${p.title}`} title="编辑" onClick={() => openEditor(p)}><IconPencil size={18} /></button>{p.origin === 'user' && <button aria-label={`删除 ${p.title}`} title="移入回收站" onClick={() => command({ type: 'delete', id: p.id, revision: d.revision })}><IconTrash size={18} /></button>}</>}</div>
            </div>; })}
          </div>
