...
const INTENT_BRIDGE_LAST_STATUSES = new Set(["unavailable", "idle", "none", "completed", "failed", "expired", "rejected", "awaiting-details"]);
...
export function createDiagnosticReport(input = {}) {
  ...
  const conversation = {
    ...
    pipeline: {
      ...
      context: {
        ...Object.fromEntries(["retainedMessages", "appliedMessages", "appliedReviewedMemories"].map((key) => [key, Math.max(0, Math.min(80, Number(pipelineSource.context?.[key]) || 0))])),
      ...
      counters: Object.fromEntries(["asrPartials", "asrFinals", "duplicateFinals", "trustedBypasses", "modelRequests", "assistantDeltas", "ttsRequests", "ttsAudioChunks", "turnsCompleted", "cancellations", "errors", "bargeInCandidates", "bargeInsAccepted", "bargeInsAcceptedExplicit", "bargeInsAcceptedPartial", "bargeInsAcceptedFinal", "bargeInsRejectedEcho", "bargeInsRejectedWeak", "bargeSpeechStarts", "bargeInsRejectedUnstable", "bargePartialsDeferred", "bargeFinalsRejectedInconsistent", "postPlaybackEchoDrops", "postPlaybackFreshSpeechStarts", "postPlaybackEchoItemDrops", "postPlaybackEchoTextDrops", "bargeFinalTimeouts", "bargeFinalRecoveries", "lateBargeFinalDrops"].map((key) => [key, Math.max(0, Number(pipelineCountersSource[key]) || 0)])),
