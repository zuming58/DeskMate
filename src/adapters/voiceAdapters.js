...
export class DesktopBridgeAdapter {
  ...
  async getMotionSettings() { return this.bridge?.getMotionSettings ? this.bridge.getMotionSettings() : { ok: false, reason: "desktop-bridge-unavailable", motionSettings: { yawAmplitudeDegrees: 20, pitchAmplitudeDegrees: 15, yawSpeedDegreesPerSecond: 80, pitchSpeedDegreesPerSecond: 80 } }; }
  async setMotionSettings(value) { return this.bridge?.setMotionSettings ? this.bridge.setMotionSettings(value) : { ok: false, reason: "desktop-bridge-unavailable", motionSettings: { yawAmplitudeDegrees: 20, pitchAmplitudeDegrees: 15, yawSpeedDegreesPerSecond: 80, pitchSpeedDegreesPerSecond: 80 } }; }
  async getMotionAutomationPolicy() { return this.bridge?.getMotionAutomationPolicy ? this.bridge.getMotionAutomationPolicy() : { ok: false, policy: { version: 1, enabled: false, idleEnabled: false }, running: false, last: { state: "disabled" } }; }
  async setMotionAutomationPolicy(value) { return this.bridge?.setMotionAutomationPolicy ? this.bridge.setMotionAutomationPolicy(value) : { ok: false, reason: "desktop-bridge-unavailable", policy: { version: 1, enabled: false, idleEnabled: false } }; }
  onMotionAutomationStatus(listener) { return this.bridge?.onMotionAutomationStatus ? this.bridge.onMotionAutomationStatus(listener) : () => {}; }
  ...
  async getCompanionConversationStatus() { return this.bridge?.getCompanionConversationStatus ? this.bridge.getCompanionConversationStatus() : { active: false, state: "idle", provider: "three-stage", audioSource: { available: false, reason: "desktop-bridge-unavailable" }, audioSink: { available: false, reason: "desktop-bridge-unavailable" }, service: { configured: false, provider: "three-stage", stages: {} } }; }
  ...
  async listMemoryTurns(value) { return this.bridge?.listMemoryTurns ? this.bridge.listMemoryTurns(value) : []; }
