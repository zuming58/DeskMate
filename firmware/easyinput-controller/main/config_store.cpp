...
esp_err_t ConfigNvsStore::begin(){
  ...
  if(err!=ESP_OK){ writable_=false; return err; }
...
ConfigLoadResult ConfigNvsStore::load(){
  ConfigSlotRecord a{},b{}; ConfigSlot marker=ConfigSlot::Invalid; uint8_t m=0;
  if(handle_&&nvs_get_u8(reinterpret_cast<nvs_handle_t>(handle_),"cfg_active",&m)==ESP_OK&&m<=1) marker=static_cast<ConfigSlot>(m);
  const bool have_a=read_slot(ConfigSlot::A,a), have_b=read_slot(ConfigSlot::B,b);
  auto selected=select_config_record(have_a?&a:nullptr,have_b?&b:nullptr,marker);
  if(selected.slot!=ConfigSlot::Invalid)return selected;
  ...
  ConfigLoadResult legacy{}; nvs_handle_t h=0;
  if(nvs_open(kLegacyNamespace,NVS_READONLY,&h)==ESP_OK){
    size_t n=0;
    if(nvs_get_str(h,kLegacyKey,nullptr,&n)==ESP_OK&&n>1&&n<=kConfigMaxJsonBytes+1){
      std::array<char,kConfigMaxJsonBytes+1> buf{};
      if(nvs_get_str(h,kLegacyKey,buf.data(),&n)==ESP_OK){ConfigProjection p{}; if(parse_config_projection({buf.data(),n-1},p)){legacy.document.length=static_cast<uint16_t>(n-1);std::copy_n(reinterpret_cast<uint8_t*>(buf.data()),n-1,legacy.document.bytes.begin());legacy.document.crc16=config_crc16_ccitt(legacy.document.bytes.data(),legacy.document.length);legacy.document.source=ConfigSource::Legacy;nvs_close(h);return legacy;}}
    }
    nvs_close(h);
  }
  const char* safe=compiled_safe_config_json(); legacy.document.length=static_cast<uint16_t>(std::strlen(safe)); std::copy_n(reinterpret_cast<const uint8_t*>(safe),legacy.document.length,legacy.document.bytes.begin()); legacy.document.crc16=config_crc16_ccitt(legacy.document.bytes.data(),legacy.document.length); legacy.document.source=(have_a||have_b||marker!=ConfigSlot::Invalid)?ConfigSource::Recovery:ConfigSource::Default; return legacy;
}
