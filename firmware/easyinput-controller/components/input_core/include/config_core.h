...
class ConfigStatusStream {
 public: bool replace(uint32_t,uint32_t,const LinkStatusSnapshot&,const AgentStateDiagnostics& = {},const AudioCaptureDiagnostics& = {}); bool encode_next(std::array<uint8_t,kConfigFeaturePayloadBytes>&) const; bool mark_sent(); void abort(); bool pending() const{return pending_;} uint32_t epoch() const{return epoch_;}
