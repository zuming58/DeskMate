...
void status_roundtrip(){ConfigStatusStream s;LinkStatusSnapshot link{};link.state=LinkControllerState::Connected;link.rx_frames=12;link.tx_frames=13;link.crc_errors=2;link.peer_restarts=1;AgentStateDiagnostics agent{};agent.accepted=5;agent.forwarded=4;agent.queue_drops=1;AudioCaptureDiagnostics audio{};audio.state=AudioCaptureState::Streaming;audio.captured_frames=101;audio.sent_frames=99;audio.dropped_frames=2;audio.read_errors=3;audio.send_errors=4;audio.recoveries=5;SpeakerOutputDiagnostics speaker{};speaker.state=SpeakerOutputState::Ready;speaker.requests=2;speaker.completed=1;speaker.cancelled_for_microphone=1;ManualCalibrationBridgeDiagnostics manual{};manual.accepted=6;manual.terminal=7;manual.conflicts=2;CHECK(s.replace(0x12345678,4,link,agent,audio,speaker,manual));std::array<uint8_t,kConfigFeaturePayloadBytes> p{};std::string json;uint16_t length=0,crc=0;uint8_t expected=0,total=0;while(s.pending()){CHECK(s.encode_next(p));CHECK(p[0]==4&&p[1]==expected++);total=p[2];length=static_cast<uint16_t>(p[9]|(p[10]<<8));crc=static_cast<uint16_t>(p[11]|(p[12]<<8));const auto count=p[3]-9;json.append(reinterpret_cast<const char*>(p.data()+13),count);CHECK(std::all_of(p.begin()+13+count,p.end(),[](uint8_t v){return v==0;}));s.mark_sent();}CHECK(expected==total);CHECK(json.size()==length);CHECK(config_crc16_ccitt(reinterpret_cast<const uint8_t*>(json.data()),json.size())==crc);CHECK(json.find("\"config_read_v1\":true")!=std::string::npos);CHECK(json.find("\"config_write_v1\":true")!=std::string::npos);CHECK(json.find("\"host_action_v1\":true")!=std::string::npos);CHECK(json.find("\"fixed_text_v1\":true")!=std::string::npos);CHECK(json.find("\"style_studio_input_lease_v1\":true")!=std::string::npos);CHECK(json.find("\"deskmate_link_v1\":true")!=std::string::npos);CHECK(json.find("\"agent_state_bridge_v1\":true")!=std::string::npos);CHECK(json.find("\"codex_led_status_v1\":true")!=std::string::npos);CHECK(json.find("\"manual_calibration_bridge_v1\":true")!=std::string::npos);CHECK(json.find("\"audio_capture_v1\":true")!=std::string::npos);CHECK(json.find("\"speaker_output_v1\":true")!=std::string::npos);CHECK(json.find("\"state\":\"connected\"")!=std::string::npos);CHECK(json.find("\"rx_frames\":12")!=std::string::npos);CHECK(json.find("\"crc_errors\":2")!=std::string::npos);CHECK(json.find("\"peer_restarts\":1")!=std::string::npos);CHECK(json.find("\"accepted\":5")!=std::string::npos);CHECK(json.find("\"forwarded\":4")!=std::string::npos);CHECK(json.find("\"queue_drops\":1")!=std::string::npos);CHECK(json.find("\"manual_calibration\":{\"accepted\":6,\"terminal\":7")!=std::string::npos);CHECK(json.find("\"conflicts\":2")!=std::string::npos);CHECK(json.find("\"audio_capture\":{\"state\":\"streaming\"")!=std::string::npos);CHECK(json.find("\"captured_frames\":101")!=std::string::npos);CHECK(json.find("\"sent_frames\":99")!=std::string::npos);CHECK(json.find("\"dropped_frames\":2")!=std::string::npos);CHECK(json.find("\"read_errors\":3")!=std::string::npos);CHECK(json.find("\"send_errors\":4")!=std::string::npos);CHECK(json.find("\"recoveries\":5")!=std::string::npos);CHECK(json.find("\"speaker_output\":{\"state\":\"ready\"")!=std::string::npos);CHECK(json.find("\"requests\":2")!=std::string::npos);CHECK(json.find("\"completed\":1")!=std::string::npos);CHECK(json.find("\"cancelled_for_microphone\":1")!=std::string::npos);CHECK(json.find("ssid")==std::string::npos);CHECK(json.find("audio_host")==std::string::npos);}

void motion_status_roundtrip(){
  ConfigStatusStream stream;LinkStatusSnapshot link{};link.state=LinkControllerState::Connected;
  AgentStateDiagnostics agent{};AudioCaptureDiagnostics audio{};SpeakerOutputDiagnostics speaker{};ManualCalibrationBridgeDiagnostics manual{};MotionPresetBridgeDiagnostics motion{};
  motion.accepted=8;motion.endpoint_acknowledgements=7;motion.malformed=6;motion.duplicates=5;motion.busy=4;motion.stale=3;motion.conflicts=2;motion.lifecycle_clears=1;motion.response_drops=9;
  CHECK(stream.replace(17,6,link,agent,audio,speaker,manual,motion));
  std::array<uint8_t,kConfigFeaturePayloadBytes> payload{};std::string json;
  while(stream.pending()){CHECK(stream.encode_next(payload));const auto count=payload[3]-9;json.append(reinterpret_cast<const char*>(payload.data()+13),count);CHECK(stream.mark_sent()||stream.pending());}
  CHECK(json.find("\"motion_preset_bridge_v1\":true")!=std::string::npos);
  CHECK(json.find("\"motion_preset\":{\"accepted\":8,\"endpoint_acknowledgements\":7")!=std::string::npos);
  CHECK(json.find("\"malformed\":6,\"duplicates\":5,\"busy\":4,\"stale\":3,\"conflicts\":2,\"lifecycle_clears\":1,\"response_drops\":9")!=std::string::npos);
...
void audio_config_projection(){
  ...
  std::string configured=kJson;configured.pop_back();configured+=R"(,"wifi_ssid":"DeskMate","wifi_password":"not-a-real-password","audio_host":"desk.local","audio_port":17333})";
  CHECK(parse_audio_capture_config(configured,audio)==AudioConfigProjectionStatus::Ready);
  CHECK(std::string(audio.wifi_ssid.data())=="DeskMate");CHECK(std::string(audio.wifi_password.data())=="not-a-real-password");CHECK(std::string(audio.audio_host.data())=="desk.local");CHECK(audio.audio_port==17333);
  std::string open=configured;const auto password=open.find("not-a-real-password");open.erase(password,19);CHECK(parse_audio_capture_config(open,audio)==AudioConfigProjectionStatus::Ready&&audio.wifi_password[0]=='\0');
  std::string wrong_type=configured;const auto port=wrong_type.find("17333");wrong_type.replace(port,5,"\"17333\"");CHECK(parse_audio_capture_config(wrong_type,audio)==AudioConfigProjectionStatus::Invalid);
  std::string duplicate=configured;duplicate.pop_back();duplicate+=R"(,"audio_host":"other.local"})";CHECK(parse_audio_capture_config(duplicate,audio)==AudioConfigProjectionStatus::Invalid);
  std::string nested=kJson;nested.pop_back();nested+=R"(,"unrelated":{"audio_host":"nested.local"},"wifi_ssid":"DeskMate","wifi_password":"","audio_port":17333})";CHECK(parse_audio_capture_config(nested,audio)==AudioConfigProjectionStatus::ConfigIncomplete);
  std::string bad_host=configured;const auto host=bad_host.find("desk.local");bad_host.replace(host,10,"http://bad");CHECK(parse_audio_capture_config(bad_host,audio)==AudioConfigProjectionStatus::Invalid);
  CHECK(parse_audio_capture_config(configured,audio)==AudioConfigProjectionStatus::Ready);CHECK(std::string(audio.audio_host.data())=="desk.local");CHECK(parse_audio_capture_config(kJson,audio)==AudioConfigProjectionStatus::ConfigIncomplete);CHECK(audio.audio_host[0]=='\0'&&audio.audio_port==0);
}
