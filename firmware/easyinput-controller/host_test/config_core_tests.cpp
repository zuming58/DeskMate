...
void status_roundtrip(){ConfigStatusStream s;LinkStatusSnapshot link{};link.state=LinkControllerState::Connected;link.rx_frames=12;link.tx_frames=13;link.crc_errors=2;link.peer_restarts=1;AgentStateDiagnostics agent{};agent.accepted=5;agent.forwarded=4;agent.queue_drops=1;AudioCaptureDiagnostics audio{};audio.state=AudioCaptureState::Streaming;audio.captured_frames=101;audio.sent_frames=99;audio.dropped_frames=2;audio.read_errors=3;audio.send_errors=4;audio.recoveries=5;SpeakerOutputDiagnostics speaker{};speaker.state=SpeakerOutputState::Ready;speaker.requests=2;speaker.completed=1;speaker.cancelled_for_microphone=1;CHECK(s.replace(0x12345678,4,link,agent,audio,speaker));std::array<uint8_t,kConfigFeaturePayloadBytes> p{};std::string json;uint16_t length=0,crc=0;uint8_t expected=0,total=0;while(s.pending()){CHECK(s.encode_next(p));CHECK(p[0]==4&&p[1]==expected++);total=p[2];length=static_cast<uint16_t>(p[9]|(p[10]<<8));crc=static_cast<uint16_t>(p[11]|(p[12]<<8));const auto count=p[3]-9;json.append(reinterpret_cast<const char*>(p.data()+13),count);CHECK(std::all_of(p.begin()+13+count,p.end(),[](uint8_t v){return v==0;}));s.mark_sent();}CHECK(expected==total);CHECK(json.size()==length);CHECK(config_crc16_ccitt(reinterpret_cast<const uint8_t*>(json.data()),json.size())==crc);CHECK(json.find("\"config_read_v1\":true")!=std::string::npos);CHECK(json.find("\"config_write_v1\":true")!=std::string::npos);CHECK(json.find("\"host_action_v1\":true")!=std::string::npos);CHECK(json.find("\"fixed_text_v1\":true")!=std::string::npos);CHECK(json.find("\"deskmate_link_v1\":true")!=std::string::npos);CHECK(json.find("\"agent_state_bridge_v1\":true")!=std::string::npos);CHECK(json.find("\"audio_capture_v1\":true")!=std::string::npos);CHECK(json.find("\"speaker_output_v1\":true")!=std::string::npos);CHECK(json.find("\"state\":\"connected\"")!=std::string::npos);CHECK(json.find("\"rx_frames\":12")!=std::string::npos);CHECK(json.find("\"crc_errors\":2")!=std::string::npos);CHECK(json.find("\"peer_restarts\":1")!=std::string::npos);CHECK(json.find("\"accepted\":5")!=std::string::npos);CHECK(json.find("\"forwarded\":4")!=std::string::npos);CHECK(json.find("\"queue_drops\":1")!=std::string::npos);CHECK(json.find("\"audio_capture\":{\"state\":\"streaming\"")!=std::string::npos);CHECK(json.find("\"captured_frames\":101")!=std::string::npos);CHECK(json.find("\"sent_frames\":99")!=std::string::npos);CHECK(json.find("\"dropped_frames\":2")!=std::string::npos);CHECK(json.find("\"read_errors\":3")!=std::string::npos);CHECK(json.find("\"send_errors\":4")!=std::string::npos);CHECK(json.find("\"recoveries\":5")!=std::string::npos);CHECK(json.find("\"speaker_output\":{\"state\":\"ready\"")!=std::string::npos);CHECK(json.find("\"requests\":2")!=std::string::npos);CHECK(json.find("\"completed\":1")!=std::string::npos);CHECK(json.find("\"cancelled_for_microphone\":1")!=std::string::npos);CHECK(json.find("ssid")==std::string::npos);CHECK(json.find("audio_host")==std::string::npos);}
