...
- Post-build read-only Windows enumeration found only the built-in serial port and the expected EasyInput USB HID interfaces; no ESP32/Xiaozhi USB download endpoint is currently present. No identity command was attempted because there is no candidate target. The operator must connect the Xiaozhi top USB-C data port before fresh identification.
- Classification: `STAGE1_REFERENCE_BASELINE_CODE_BUILD_CONFIRMED / XIAOZHI_USB_NOT_PRESENT / FLASH_NOT_AUTHORIZED / SERVO_MOTION_NOT_RUN`. No Flash/NVS/eFuse, erase, flash, monitor, OLED, audio, PWM, GPIO or servo operation occurred. Next: after the Xiaozhi top USB-C is connected, identify that single endpoint read-only, then request one explicit app-only flash authorization before the user-present single-axis test.
