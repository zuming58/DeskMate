...
## 2026-09-07 - Selected-microphone wake bridge, bounded realtime startup and split speech volume packaged

- The Windows-only implementation is committed on `codex/t18-software-closure` at `402240bc7038c20c088152be9f5fa6545b55878f` and awaits user microphone/audio HIL. Ordinary Voice Input was deliberately left on its accepted explicit toggle contract: first press starts capture, second press stops and submits. No silence auto-stop or second VoiceWorkflow was introduced.
- The background wake path no longer asks the hidden Windows `System.Speech` process to open an unrelated system-default microphone. When idle and explicitly enabled, the renderer opens the same computer microphone selected in DeskMate, streams bounded mono PCM16 to the local recognizer and exposes only privacy-safe received/rejected counters. The first user launch exposed that `System.Speech.SetInputToAudioStream` rejects the raw stdin pipe because it is non-seekable; the child exited and an unhandled asynchronous `EPIPE` crashed Electron main. A bounded read-only seek facade now satisfies the recognizer without buffering audio, stdin errors are always consumed, writing stops as soon as the child disconnects and existing finite recovery remains in force. A real five-second local PCM smoke kept the listener alive with no child exit or error. Dictation, microphone test and realtime companion still preempt wake; stopping, device change and renderer loss close both ends without replay or retained audio.
- Initial Doubao realtime connection now receives one bounded attempt instead of consuming the full runtime reconnect schedule while the capsule remains at “connecting”. Runtime connection loss keeps the existing bounded reconnect policy. A privacy-safe live check using the installed encrypted DeskMate configuration reached `connection.started` and `session.ready` on the configured Doubao realtime endpoint; no key, audio or transcript was printed or persisted.
- Companion audio now has two independent local gains: normal AI companion speech defaults to `75%`, while proactive Codex waiting/completed/error announcements default to `30%`. Both are editable in AI Companion settings in 5-point steps, do not change Windows system volume and are restored deterministically after a low-volume task announcement inside an active conversation. Existing preferences migrate without losing name, wake phrase or timing settings.
- Verification: `npm ci --include=dev`; full `npm test` `425/425`; `npm run build:desktop`; live Doubao session-ready check; real five-second Windows wake PCM pipe smoke; `git diff --check`. Fresh unpacked runtime `release/win-unpacked/DeskMate.exe` is 202690560 bytes / SHA-256 `8D14D26A03C09D5D8212CC49BBACA070E20B7DBE5DAB199DCEA823D86FF26A5E`; `app.asar` is 113137428 bytes / `8EC5617F3F34AAA11A26FDCEE1CA0B49A998CCDAEB11A80A32D3D7AD152390F3`. This exact package was relaunched and its Electron process tree remained alive after startup.
- User-present acceptance is still required for the actual selected microphone, configured wake phrase, perceived normal/reduced volume and one real Codex proactive announcement. No firmware, Host HID, DeskMate Link, device discovery, serial/HID write, Flash/NVS/eFuse or servo operation changed or occurred.
- Classification: `WINDOWS_CANDIDATE_BUILT_AND_LAUNCHED / DOUBAO_SERVICE_HANDSHAKE_CONFIRMED / SELECTED_MIC_WAKE_AND_SPLIT_VOLUME_HIL_PENDING / ORDINARY_DICTATION_TOGGLE_UNCHANGED / FIRMWARE_UNCHANGED / NO_DEVICE_WRITE`.

## 2026-09-07 - Wake hot-reload repaired; Codex status decoupled from Xiaozhi face

- Windows implementation is committed on `codex/t18-software-closure` at `02df0c349e848b619f27c7f22cfd406648f4b80c`; bounded listener recovery and pause-reason UI are committed at `a0bd05a85c9db30558c0bd689126b28ffb9dc2b2`.
- User HIL found two Windows defects in the resumed T20 package. Saving a new companion name/wake phrase changed the persisted UI value but the already-running Windows `System.Speech` child remained alive with the old grammar. Separately, the default selected Codex lifecycle provider kept publishing `working` after the user manually selected `idle`, so Xiaozhi returned to the square-eye work expression.
- `WindowsSpeechWakeWordAdapter` now detects a live phrase-set change, awaits termination of the old listener and starts a replacement immediately while audio is idle. Punctuation/whitespace-free variants are included within the existing eight-phrase bound and the short-phrase confidence floor is `0.50`. The settings page now distinguishes immediately applied idle wake grammar from next-session name/endpointing, and explicitly states that this local engine uses the Windows system-default microphone.
- Codex lifecycle remains connected for trusted task lists, deterministic status queries and enabled waiting/completed/error announcements, but its hardware-expression route now defaults to `disabled`. Provider events no longer mutate renderer face state or publish Agent State to Xiaozhi. Xiaozhi stays at idle when no companion conversation is active; listening/thinking/speaking expressions remain companion-owned, and the seven-state buttons remain explicit manual HIL only. Future five-LED Codex indication is a separate, not-yet-frozen feature and was not guessed here.
- Unexpected local listener termination now receives at most three bounded retries with 0.5/1/2-second backoff; an intentional foreground pause cancels pending recovery. The Companion page distinguishes foreground companion/dictation ownership, recovery and terminal failure instead of showing only a generic pause label.
- Verification: focused wake/manual/expression tests `19/19`; full `npm test` `422/422`; `npm run build:desktop`; packaged InputBridge `--protocol-self-test`; `git diff --check`. Fresh unpacked artifacts: `DeskMate.exe` 202690560 bytes / SHA-256 `AD244DD471D08E6ACD22A2B81BB4CA574B14BD10343D819CBAB8A909E722AC24`; `app.asar` 113121108 bytes / `9A0946B6069D5B2A01A4C94246E3B09AC6706494BD8E816D5A9CB0CFB525B47A`; InputBridge 153525129 bytes / `4B0D8194FB771E4D5CCCFED20CDDE0BE64F4FF7B64AFCC270B56A941B80B9E18`.
- The exact fresh package was launched. Its wake listener was absent during the initial foreground startup window, recovered by 13 seconds and remained alive at 23 seconds. This proves bounded recovery and stable local process lifetime, not that real speech reached the system-default microphone. Next HIL: save a new repeated wake phrase while idle, verify the page reaches `正在监听`, speak it, then end the session and confirm the capsule hides; also send manual `待命` once and confirm no Codex work event restores square eyes.
- Classification: `WINDOWS_REPAIR_BUILT_AND_LAUNCHED / WAKE_PROCESS_ALIVE / USER_REAL_SPEECH_AND_IDLE_FACE_HIL_PENDING / CODEX_TASK_MONITOR_RETAINED / CODEX_FACE_ROUTE_DISABLED / FIRMWARE_UNCHANGED / NO_DEVICE_FLASH`.

## 2026-09-07 - Exact T20 package relaunched for resumed user HIL

- After a long pause in user testing, the current source-of-truth was rechecked as `codex/t18-software-closure@132bc6db9e9084902b1e9c60fedb7debe6310dbc`; implementation remains `fef3ebdaaa8b8ef937d531fbc7a2aa08d15d0bcf`. The accepted T15D V2 EasyInput/Xiaozhi firmware baseline remains unchanged and requires no reflash.
- The packaged `release/win-unpacked/DeskMate.exe` was re-hashed as `11FA6E73B497DD5B62026F06894ADD73F610E39475D0AC6B1A0AFADC0907F65A`, matching the T20 build record, and launched successfully. The expected Electron process tree is running from this exact workspace package.
- Resumed HIL order: hidden idle with no capsule; exact “小智，小智” wake; roughly four-second end-of-utterance response and ten-second listening-only foreground exit; repeated wake; fixed/custom motion with generated sound; then trusted aggregate/named Codex status and proactive waiting/completed/error announcements. T17 raw-turn/daily-summary/search/review/export/forgetting remains a separate later acceptance pass.
- Classification: `EXACT_T20_RUNTIME_RELAUNCHED / USER_WAKE_TIMING_MOTION_AUDIO_AND_CODEX_HIL_PENDING / FIRMWARE_UNCHANGED / NO_DEVICE_WRITE`.

## 2026-09-07 - Single-battery hardware feasibility audit; no wiring authorized

- The user reopened only a bounded mechanical/power integration question: whether the larger EasyInput battery can supply both EasyInput and the Xiaozhi gimbal so the product does not need two power USB cables. This is an analysis request, not authorization to wire, solder, power, flash or modify either board.
- Existing evidence confirms EasyInput uses a one-cell `VBAT`/TP4057 charge path and does not expose a documented high-current 5 V output. Xiaozhi has a photographed `3.7 V 800 mAh 2.96 Wh` cell, two `MG90S`-labelled servos, a tutorial-described single-charge/single-discharge power module and a source-described 5 V servo rail. Exact servo stall current, EasyInput cell discharge rating, charge/load-sharing behavior, rail droop and backfeed paths remain unmeasured.
- Feasibility conclusion: one battery and one external connector are practical only through a new central protected power tree. Do not parallel the two battery connectors/chargers or feed Xiaozhi from EasyInput J4 `3V3`, GPIO8 peripheral rail or an undocumented board 5 V point. The preferred product topology is one protected 1S cell plus a true load-sharing charger/power-path stage and a separately protected high-current 5 V converter/distribution branch for Xiaozhi/servos, with a separate EasyInput branch and common logic ground.
- Before selecting parts or approving a prototype, record the EasyInput battery label/protection rating and both power boards' clear front/back photos; map unpowered continuity; then measure independent idle/startup, dual-servo motion/stall-bound transient, audio, charge-and-run and USB plug/unplug behavior under current-limited bench power. Size the cell, converter, wiring, protection and bulk decoupling from measured peak current and voltage sag, not battery capacity alone.
- Classification: `READ_ONLY_HARDWARE_AUDIT / SINGLE_POWER_FEASIBLE_WITH_NEW_POWER_TREE / EXISTING_BOARD_TO_BOARD_POWER_UNSAFE / POWER_PATH_MEASUREMENT_PENDING / NO_HARDWARE_OPERATION`.

## 2026-09-04 - T20 hidden background wake, fast foreground exit and built-in action audio packaged

- The user's distinction between the two timers is now the frozen Windows behavior. Provider endpointing defaults to four seconds of end-of-utterance silence, while the separate foreground listening-only idle timer defaults to ten seconds. Idle expiry closes the Doubao session, releases foreground audio, hides the capsule and resumes enabled local wake; it never makes a completed sentence wait ten or sixty seconds.
- Windows `System.Speech` `zh-CN` remains the local-only wake boundary. Idle wake lifecycle/status events no longer render the capsule, exact configured phrases include `小智小智`, and duplicate recognition callbacks are suppressed for three seconds. Dictation, microphone test and realtime companion still preempt the listener. A real local two-second process smoke returned `available=true`, `enabled=true`, `mode=background-local`, `capsuleVisible=false` and released the microphone without retaining audio or text.
- The external user preference at `%APPDATA%/deskmate/companion-preferences.json` was updated only after the user's explicit request: `wakeEnabled=true`, `endSmoothWindowMs=4000`, `idleTimeoutMs=10000`. Repository defaults use the same timing but keep wake opt-in (`false`) for other installations.
- Every frozen motion preset now has a locally generated copyright-free computer-speaker cue. Attention, nod and search use short gentle cues; dance uses an eight-second electronic beat when no enabled local song exists. An enabled selected local track still overrides only dance, while generic “播放音乐” falls back to the built-in beat. Completion, stop, supersession and emergency stop retain the existing audio-generation cleanup.
- Windows implementation is `codex/t18-software-closure@fef3ebdaaa8b8ef937d531fbc7a2aa08d15d0bcf`. Full `npm test` passes `419/419`; `npm run build:desktop`, `git diff --check` and packaged InputBridge `--protocol-self-test` pass. Fresh unpacked artifacts: `DeskMate.exe` 202690560 bytes / SHA-256 `11FA6E73B497DD5B62026F06894ADD73F610E39475D0AC6B1A0AFADC0907F65A`; `app.asar` 113119143 bytes / `CEA3B9B7D946DDD093C1C3506F25FA60F9E96A4DD8B4FC37DE62FC685BDAEEA2`; InputBridge 153525129 bytes / `D0EC796302A0FDCDD658DA41F89EEE807D883253BA8558FB714F10460C66B3EB`.
- The first packaging attempt was deliberately interrupted after it stopped producing progress; its incomplete output was isolated as `release/win-unpacked-incomplete-t20`. A clean rerun completed and only the fresh `release/win-unpacked` directory is eligible to launch or test.
- The exact fresh `release/win-unpacked/DeskMate.exe` was launched after the preference update. Process inspection found the expected workspace Electron process tree and one Windows PowerShell wake-listener child owned by the main process; this proves the enabled local listener is alive, but phrase recognition, capsule appearance and perceived timing still require user observation.
- No firmware source, Host HID, DeskMate Link, device discovery, serial/HID write, Flash/NVS/eFuse operation or servo contract changed. Classification: `T20_WINDOWS_CODE_BUILD_CONFIRMED / LOCAL_WAKE_PROCESS_SMOKE_PASSED / EXACT_RUNTIME_LAUNCHED / USER_WAKE_TIMING_AND_AUDIO_HIL_PENDING / FIRMWARE_UNCHANGED`. Next: verify hidden idle, “小智，小智” wake, roughly four-second answer start, ten-second foreground disappearance, repeated wake and one built-in dance beat.

## 2026-09-04 - T19 deterministic application launch and local dance music packaged; hardware/ecosystem scope cancelled

- The user explicitly removed hardware expansion, face tracking, speaker/person identification and every non-Codex Agent from the active product roadmap. A new EasyInput speaker-downlink project is also cancelled; these items may return only through a new explicit product decision. The accepted T15D V2 firmware on EasyInput and Xiaozhi remains frozen and was not rebuilt, accessed or written.
- Windows implementation is committed on `codex/t18-software-closure` at `20f893da77056293dca0c38fa384b7d65cfd5a0f`. “智能控制” is now the single visible place to discover/search local Start-menu applications, add one registration and explicitly allow voice opening. Utterances such as “打开网易云音乐” resolve deterministically to one longest registered label before any language-model classifier; negation, disabled policy, missing target and equal-strength ambiguity fail closed.
- T19 adds one optional local dance track. The user selects MP3/WAV/M4A/AAC/FLAC/OGG up to 32 MiB; only Windows `safeStorage` ciphertext contains its path, while React receives a bounded label and playback bytes. Built-in dance, activated custom dance and explicit voice dance share one computer-speaker playback generation; normal completion, stop-and-center and emergency stop all stop the music. “播放音乐/停止音乐” control this same selected local file. Streaming-service search/control and generated singing are not claimed.
- The Motion page now exposes choose/replace, preview/stop and one “跳舞同步播放音乐” switch. Application and media capabilities remain Windows-only and introduce no Host HID, DeskMate Link, PWM, GPIO, servo or firmware change. Frozen contract: `T19_LOCAL_APP_MEDIA_V1_FROZEN`.
- Verification: all changed CJS files pass `node --check`; focused T19/application/choreography tests pass; full `npm test` passes `417/417`; `npm run build:desktop` passes after closing only the exact old workspace runtime that held the output directory; `git diff --check` passes. Fresh unpacked artifacts: `DeskMate.exe` SHA-256 `6BC92A19D109FD2FDDB4C40412CD7770B010608653A7CE0FAE846F0DF8F0D00A`; `app.asar` SHA-256 `16B8440821C6DDA0160CF49BAD1D0C36271ECE24DB5BFBD5D71E7DAC84688176`.
- The exact new unpacked runtime at `release/win-unpacked/DeskMate.exe` was launched after packaging. No stale workspace runtime remained during the build, and the persisted DeskMate user-data directory was retained.
- Classification: `T19_WINDOWS_CODE_BUILD_CONFIRMED / APPLICATION_AND_MUSIC_USER_HIL_PENDING / FIRMWARE_FROZEN_UNCHANGED`. Next user-present gate: in `AI 陪伴 -> 智能控制`, search/add 网易云音乐 and verify the exact enabled app opens by voice; in `AI 陪伴 -> 动作编排`, select a local song and verify preview, quick dance, custom dance, voice dance, stop and emergency stop. T17 long-term memory and T12/T15C companion regression remain the next software-only acceptance work.
