...
- What changed: from exact baseline `8d6af0cd38fb3fed85ceba03bcd99857dd1e552e`, branch `codex/xiaozhi-t10c-manual-calibration` froze additive `T10C_MANUAL_CALIBRATION_LINK_V1_FROZEN` plus golden vectors and added a pure C++ manual owner, disabled/fake servo adapters and optional simulated endpoint injection. Base DMLK framing/CRC/UART and existing messages are unchanged.
- Safety semantics: output requires selected axis plus a volatile 1000..5000 ms one-use ARM token; only adapter-local provisional center, fixed 1.0-degree direction step and recenter exist. Emergency stop is highest priority and idempotent. Lease expiry, disconnect and peer restart disarm and clear pending state without replay. Independent terminal response/status support requested/forwarded/completed-or-rejected evidence; no endpoint result is called physical-angle proof.
- Production boundary: `app_main` injects no manual owner, the real adapter is absent, and MOTION capability stays disabled. Reference-board GPIO11/GPIO12/50 Hz is documented but installed mapping, supply/current/common ground, center, direction, limits and cutoff remain `UNKNOWN`.
- Verification: final implementation `b1d43901a6b4ddc3009889cd1aa3c3b9669f8736` (core `ef7f6732485411e932c0bd1cf2a8c361e9c9ac62`); Xiaozhi Host CTest 11/11; exact ESP-IDF v5.5.3 `2c211b236707889e8400c4dc5644dd5c4ee071e0`, `esp32s3`, fixed 16 MiB partition code-gate build passed. Build ID `ef7f673`; app 206,432 bytes (`0x32660`) at `0x100000`, SHA-256 `C2C22098A754D0FC1C1AAE1096C3FE6C964C0798E4100DD84D59796651BFED7E`; partition SHA-256 `4D122CA60C7321C2C4CB393D3B612908263C2C860E92EDD43036FDBFD1C762E0`. Final documentation HEAD requires a clean receiving rebuild because the Git version is embedded.
- Stop gate: no port/device/Flash/NVS/eFuse/flash/erase/monitor/OLED/audio/PWM/GPIO/servo operation occurred. Windows/EasyInput motion forwarding remains separately owned and not frozen. Provenance: `docs/provenance/t10c-xiaozhi-manual-calibration-provenance.md`; handoff: `docs/handoffs/t10c-xiaozhi-manual-calibration-candidate-2026-09-01.md`. Push this code-only branch and stop for cross-audit; T10B Stages 0..3 remain `NOT_RUN`.
