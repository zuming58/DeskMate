...
- Root cause and fix: ESP-IDF v5.5.5 declares `i2s_channel_read(..., uint32_t timeout_ms)` and converts that value with `pdMS_TO_TICKS` internally. DeskMate incorrectly passed `ticks(kMicrophoneReadTimeoutMs)`, so the configured 80 ms became 8 ms at `CONFIG_FREERTOS_HZ=100`, shorter than one 20 ms PCM frame. Implementation commit `72fe9129c10f2567df09ef0997613d69d8d2cec2` now passes the 80 ms value directly, matching the fixed Maker reference `7619bd13f9ddfd6e2d80e2b8e022ef0acf32ce01`; a source-contract regression rejects any restored pre-conversion.
- Verification: EasyInput Host CTest 10/10 passed. A clean isolated ESP-IDF v5.5.5 `esp32s3` build from implementation commit `72fe912` passed with the fixed 16 MiB partition SHA-256 `7C541B70DCAC8F920C2D11589F06745E1B033FA9B95B8343DE2748BB8312A278`. The app is 854,416 bytes (`0xD0990`), SHA-256 `743151933B24849BB182024750217FF4648CDB31A93531A91339C44256B0AEDC`, data range `0x010000..0x0E098F`, covering sectors through `0x0E0FFF`, with 73% of the 3 MiB factory partition free. `git diff --check` and AGENTS/CLAUDE identity checks passed.
- Safety and next: no port scan, device identification, Flash/NVS access, write, erase, monitor, eFuse or Xiaozhi operation occurred. Status is `I2S_TIMEOUT_FIX_CONFIRMED / TEST_CONFIRMED / BUILD_CONFIRMED / HIL_REPAIR_PENDING`. Obtain a fresh app-only authorization for the exact implementation commit and image above, then repeat the 30-second microphone test and require rising audio-frame count and non-zero level while speaking.
...
- Recovery evidence: the fixed partition table, complete 24 KiB NVS and complete 3 MiB factory app partition were backed up to Git-external `F:\Codex\deskmate-device-backups\easyinput\t10e-preflash-20260831T135352`. The partition table matches the product's fixed 16 MiB layout byte-for-byte over its effective content, its read padding is erased, the factory partition begins with the independently read T09 app, and NVS was never parsed. The privacy-safe manifest SHA-256 is `036D655B736A0AA2D59E8F8A96C24C8B4A7ED5D24151B361DBF24B28845D17A1`.
- Final candidate: the clean pushed source HEAD `77fabec3f2fed2e4d103c058140ccdaa7c97048a` was rebuilt with exact ESP-IDF v5.5.5. The app is 854,432 bytes (`0xD09A0`), SHA-256 `88D392070591E5E04862730A0DECB0CE7954E98BEF0650EF6744BBA96F95A642`, data range `0x010000..0x0E099F`, and covering write sectors would end at `0x0E0FFF`. The earlier dirty-tree candidate was rejected and is not authorized for use.
- Safety and next: no Flash write, erase, eFuse operation, NVS parse/write, partition change, monitor session or Xiaozhi operation occurred. Status is `PRE_FLASH_BACKUP_CONFIRMED / APP_FLASH_NOT_AUTHORIZED`; wait for a separate confirmation naming this exact source HEAD, image hash and range before any write.
...
- Verification: EasyInput Host CTest 10/10, exact ESP-IDF v5.5.5 `esp32s3` fixed-partition build, desktop 127/127 and desktop package all pass. The final clean rebuild from pushed source HEAD `77fabec3f2fed2e4d103c058140ccdaa7c97048a` is 854,432 bytes (`0xD09A0`), SHA-256 `88D392070591E5E04862730A0DECB0CE7954E98BEF0650EF6744BBA96F95A642`, with 73% of the 3 MiB factory app partition free. Static scope, provenance/license, high-risk secret, ASCII path, build-artifact, `git diff --check` and AGENTS/CLAUDE checks pass.
