...
## 2026-09-02 - T10D-B Windows manual calibration safety UI complete; physical motion remains blocked

- Scope and baseline: Windows software only on `codex/t10d-desktop-manual-calibration-ui`, created in the isolated worktree `F:\Codex\deskmate-t10d-desktop-manual-calibration-ui` from cumulative software HEAD `35e627389282d8279d82646787f509681474c048`. It imports the frozen T10D-A host contract and vectors from EasyInput HEAD `1645bf688b11d2f0d7ba3dfa7900f552886cb404`; implementation commit is `695c47d255ccfc8b09e1fd2e9644735b7c0c1017`. The user's dirty primary worktree was restored and left unchanged.
- Delivered: strict `0x16/0x17` host codecs, .NET pre-write/pre-relay validation, one request in flight, per-mount request IDs, separate confirmation IDs, status-before-command gating, four explicit ARM attestations, one-use volatile authorization, yaw/pitch selection, provisional center, fixed `-1/+1` degree step, recenter, emergency stop and clear. No absolute angle, arbitrary step, PWM, pulse-width, duty-cycle or GPIO field exists.
- Truth and safety: the UI shows user intent, EasyInput accepted and Xiaozhi terminal as three independent evidence layers and includes `completed_output_count`; it never calls accepted a movement. Current production Xiaozhi `NOT_READY` is expected, closes the gate and cannot be presented as servo readiness. USB disconnect/remount clears pending work, context and authorization.
- Verification: `npm ci --include=dev` installed `398` packages; focused T10D-B/native tests passed `14/14`; full `npm test` passed `283/283`; `npm run build:desktop` and `git diff --check` passed. Firmware-boundary, ASCII tracked-path and secret-pattern checks passed. `DeskMate.exe` is `202690560` bytes / SHA-256 `2DD0ECB13782AE5287977A13A34EFAA9711D7655D71DF67A6C1364EF0428F101`; `app.asar` is `112760685` bytes / SHA-256 `E03DB4A22E3695496108159FDAF4F34E3708713D3AF7EECDE3497962E23150E1`.
- Safety and next: no application launch/control, port/device access, firmware, Flash/NVS, OLED, servo, audio-hardware or physical-motion operation occurred. Status is `CONTRACT_FROZEN / TEST_CONFIRMED / BUILD_CONFIRMED / HIL_NOT_RUN`. T10D-C requires a production Xiaozhi manual owner/adapter plus explicit user-present Stage 0 electrical/mechanical authorization; do not run physical calibration before that gate. Contract: `docs/contracts/t10d-b-desktop-manual-calibration-v1.md`; handoff: `docs/handoffs/t10d-b-desktop-manual-calibration-ui-2026-09-02.md`.

## 2026-09-02 - T13 persona, reviewed memory, knowledge projection and safe intent software complete

- Scope and baseline: Windows software only on `codex/t13-desktop-persona-memory-intent`, created in the isolated worktree `F:\Codex\deskmate-t13-desktop-intelligence` from exact base `710595f0b8b4bd209721fef9c6a96d5b80f43481`. Implementation commit is `04f1fc06e0021fd44dbe2a9ba99bcadb599714bf`; the user's dirty primary worktree was not changed.
- Delivered five software slices: a versioned persona frozen per new Doubao session; explicit real-turn memory generation into review-required candidates; accepted-memory chunking, deterministic local embeddings, bounded hybrid retrieval and conflict-preserving Markdown double-link projection; a closed intent bridge that can only propose registered application actions or bounded Codex-status queries and requires a one-use visible confirmation; and privacy-bounded Codex lifecycle summaries that never invent percentage progress.
- Trust and privacy boundary: SQLite remains the source of truth. Only reviewed and accepted memories may enter a later new conversation. Generated candidates never auto-accept, Markdown/index files are disposable derivatives, and external edits are preserved as conflicts. Paths, commands, URLs, raw vectors, credentials, transcripts, PCM and device identity do not enter renderer diagnostics or the action model. No action runs before confirmation.
- Verification: `npm ci --include=dev` installed `398` packages; full `npm test` passed `276/276`; `npm run build:desktop` passed; `git diff --check`, ASCII changed-path, ignored-output and package checks passed. `DeskMate.exe` is `202690560` bytes / SHA-256 `0A0A71CD547C4CFAE7381620B1658428EC46D459A17A221B2EAC1264398EF384`; `app.asar` is `112721657` bytes / SHA-256 `9CD874363D25830A3AF68BBFE45DA6434F270BCF83C274EFECFD146D5B7FFB16`.
- Safety and next: no application launch/control, user data, credential, port/device, firmware, Flash/NVS, OLED, servo or hardware operation occurred. Status is `TEST_CONFIRMED / BUILD_CONFIRMED / USER_ACCEPTANCE_PENDING`. User-present acceptance must cover persona tone/boundaries, review-correct-accept-forget, managed knowledge-root conflicts, confirmed application actions and real Codex lifecycle copy. Contract: `docs/contracts/t13-desktop-persona-memory-intent-v1.md`; handoff: `docs/handoffs/t13-desktop-persona-memory-intent-2026-09-02.md`.
...
- Root audit and repair: the outgoing ASR object combined the documented `end_smooth_window_ms` with an unrelated `enable_asr_twopass=true` override and an additional `audio_info` block that are absent from the official realtime dialogue sample. Commit `8f9220fc7db05c20abd1853af83ce59652b6ab3e` reduces `StartSession.asr` to the single bounded endpointing field. The already HIL-selected `dialog.extra.input_mod=keep_alive`, strict half-duplex, event boundaries, microphone selection, idle timeout and physical companion-call path are unchanged.
- Product truth: the setting copy now says it is a provider-side endpointing request sent only with a new session, not a local delay or proof that the provider accepted the requested duration. Spoken wake remains explicitly unavailable; the EasyInput `AI 陪伴呼唤` Host Action remains the supported physical entry.
- Verification: `npm ci --include=dev` installed `398` packages; focused endpointing/controller/runtime tests passed `60/60`; full `npm test` passed `270/270`; isolated `npm run build:desktop -- --config.directories.output=release-t12b1-provider-endpointing` passed; `git diff --check`, ignored-output, ASCII tracked-path and packaged build-ID checks passed. `DeskMate.exe` is `202690560` bytes / SHA-256 `169C290A0C43375634338353C3FCAC1CC607B13F286291A78E6A88DF6783F32C`; `app.asar` is `112680073` bytes / SHA-256 `71E3AAC9E708AB97815591A3D4D7E278DA3B5830C835D84FFEC0F0FC0F8F0012`.
- Safety and next: no application control, credential, transcript, PCM, port/device, firmware, Flash/NVS, OLED, servo or hardware operation occurred. Status is `TEST_CONFIRMED / BUILD_CONFIRMED / HIL_PENDING`. A new exact-package session must verify one eight-second mid-utterance pause. If it still finalizes early, export one new sanitized diagnostic; do not add another guessed provider flag or local artificial response delay.
...
- T12A evidence audit: the user-present run accepted the physical EasyInput call start/answer interruption and practical idle behavior; its sanitized summary had saved `5000/60000` ms, one session, nine user finals, nine TTS turns and no provider/transport/dialog error. T12A could not distinguish saved from active-session values, measure partial-to-final time or retain post-completion idle-stop counters, so those evidence gaps were not promoted to acceptance.
- Product repair: the Companion overview now uses independent self-height columns and a bounded `3:2` face. Settings are a draft plus one explicit validate/write/readback Save action. Pause accepts 0.5–50 seconds by 0.5; idle stop accepts 0/off or integer 10–3600 seconds. Each new session freezes one revision, including identity, provider endpointing and local idle policy, through reconnects.
- Evidence repair: diagnostics separate saved and session-applied numeric parameters, expose a content-free provider partial-to-final interval/sample count and receive final stop lifecycle evidence after an internal idle stop. Non-state lifecycle metadata converges in the renderer without reopening the overlay; name, wake phrase, text, audio and identifiers remain excluded.
- Verification: `npm ci --include=dev` passed; full `npm test` passed `267/267`; `npm run build:desktop` passed; packaged build ID inspection passed. `DeskMate.exe` is `202690560` bytes / SHA-256 `8C53F101492C35B279DCEB6BE7AF75108A5CB098E3BEA28A2C74D4248C5811A3`; `app.asar` is `112679861` bytes / SHA-256 `5ED1632252AB8DB287FE2EC67FC058ECC379DD497760243EA0B7EC3A56C92271`. `git diff --check`, ASCII paths and firmware/native source boundaries passed.
- Safety and next: the T12B package was not launched. Before the active task was corrected after a context handoff, existing T12A processes were mistakenly restarted once; no further application control occurred. No device, port, credential, text, audio, firmware or hardware access occurred. Status is `TEST_CONFIRMED / BUILD_CONFIRMED / HIL_PENDING`. Contract: `docs/contracts/t12b-companion-layout-timing-settings-v1.md`; audit: `docs/reviews/t12b-t12a-hil-evidence-and-layout-audit-2026-09-01.md`; handoff: `docs/handoffs/t12b-companion-layout-timing-settings-2026-09-01.md`.
...
- Accepted HIL and root cause: T11D.5 kept one connection with no provider/transport/dialog failure, but reported 11 TTS starts versus 10 ends, 233 accepted/226 played/7 cancelled blocks and 11 interruptions while the user heard one intermediate answer truncate. Code audit proved that provider arrival classified ASR from handler-delayed controller state, allowed uplink during `thinking` and local drain, and unconditionally interrupted sink/provider state on every accepted ASR final. Back-to-back TTS/ASR arrivals could therefore admit delayed/reflected ASR and cancel queued playback.
- Repair and diagnostics: one synchronous closed half-duplex arrival phase now advances before event queueing. Only `listening` forwards PCM/accepts ASR; `thinking`, `speaking` and `draining` suppress both until every local playback block completes and sink drain acknowledges. Normal ASR final no longer interrupts; only the explicit interrupt control may cancel an active answer. Diagnostics add content-free TTS started/completed-after-drain/abandoned counts, ASR-final arrival-phase counts, chat-final/TTS-end pairing and sink-cancel causes.
- Verification: `npm ci --include=dev` passed; focused controller/audio/terminal/privacy regression passed `72/72`; full `npm test` passed `253/253`; `npm run build:desktop` passed. Isolated `DeskMate.exe` is `202690560` bytes / SHA-256 `B8A8CC0D27365354CF595C9B193E7B77C30D668FCD12C96EF9CB9614D6D941C2`; `app.asar` is `112651386` bytes / SHA-256 `6401471B58C8D4B143A73B5EB214C5E2399523215E4479016203EF387AA5DFE9`. Read-only package inspection confirmed the build ID; `git diff --check` passed.
- Safety and next: no application launch/control, credential, transcript, reply, PCM, provider payload, port/device, firmware, Flash, NVS, OLED, servo or audio hardware access occurred in this package. Status is `TEST_CONFIRMED / BUILD_CONFIRMED / HIL_NOT_RUN`. User-present HIL must run one multi-sentence answer without automatic interruption, a second turn in the same session, one explicit manual interruption and one final stop, then export one sanitized diagnostic. Contract: `docs/contracts/t11d6-turn-cancel-diagnostics-v1.md`; review: `docs/reviews/t11d6-intermediate-answer-truncation-root-cause-2026-09-01.md`; handoff: `docs/handoffs/t11d6-turn-cancel-diagnostics-2026-09-01.md`.

## 2026-09-01 - T11D.5 half-duplex keep-alive root repair and Companion layout complete

- Scope and identity: Windows software only on `codex/t11d5-dialog-error-root-ui`, created from exact T11D.4 base `3a62bf123e19fcaaf48ded9db0b9e41144f523bc`; implementation commit is `9d7e9e873ef4ea43c67f0ec127b03361e2489a13`. Build identity is `t11d5-half-duplex-keepalive-ui-v1`.
- Root cause: exact T11D.4 HIL had eight complete TTS turns, 136/136 accepted/played blocks, zero queue, drain, cancellation, backpressure or transport failures, but 600 strict echo-guard upstream drops before an adjacent draining-phase `DialogCommonError`. Current official Volcengine documentation requires `dialog.extra.input_mod=keep_alive` when microphone upload can pause and defines `52000042 DialogAudioIdleTimeoutError`. The rejected diagnostic did not export the numeric code, so the category match is explicitly an evidence-supported inference.
- Repair: StartSession now declares `keep_alive` while preserving strict half-duplex, local playback drain, same-session event `359` and fail-closed event `599`. Official status `52000042` maps only to `audio-idle-timeout`; raw provider data remains excluded. The Companion action/notices/evidence are grouped in one normal-flow stack, with single-column small-window behavior and no absolute or negative-position workaround.
- Verification: `npm ci --include=dev` passed; targeted protocol/controller/diagnostic/UI tests passed `44/44`; full `npm test` passed `248/248`; isolated Windows packaging passed. `DeskMate.exe` is `202690560` bytes / SHA-256 `7ADEA1A62B75B9D5936D33AD3AD60D0B903DFC3D7629AE98EBBD547533B998EF`; `app.asar` is `112643359` bytes / SHA-256 `75B15AD60882857031C3FBDFA2571CAA190C39497830C99BDE8AD09E16F94CF4`. Read-only package inspection confirmed the build identity; `git diff --check`, firmware boundary and ignored-output checks passed.
- Safety and next: no application launch/control, user diagnostic, credential, audio/text, device, port, Flash, firmware or hardware access occurred. Status is `TEST_CONFIRMED / BUILD_CONFIRMED / HIL_NOT_RUN`. User-present HIL must play one long response and a second turn without automatic stop/reconnect/welcome replay, and visually confirm the action no longer overlaps the three evidence cards. Contract: `docs/contracts/t11d5-half-duplex-keepalive-v1.md`; review: `docs/reviews/t11d5-dialog-audio-idle-timeout-root-cause-2026-09-01.md`; handoff: `docs/handoffs/t11d5-half-duplex-keepalive-ui-2026-09-01.md`.
