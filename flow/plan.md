...
- T11E-A EasyInput 本地扬声器硬件底座已按 [`EASYINPUT_SPEAKER_OUTPUT_V1_FROZEN`](../docs/contracts/easyinput-speaker-output-v1.md) 完成代码与构建门：I2S1 固定 GPIO14/13/15、48 kHz/16-bit/mono-left，复用 GPIO8 `Speaker` 租约，以一次低音量合成开机双提示音作为后续真机门，并用 generation 仲裁保证 T10E 麦克风绝对优先。Host 12/12 与 ESP-IDF v5.5.5 固定 16 MiB 分区构建通过；当前仍为 `HIL_NOT_AUTHORIZED`，未访问设备、Flash、NVS 或声音 bank。实时桌面扬声器下行仍未冻结，不得把本地探针描述为实时对话已接通。
- T10E EasyInput 板载麦克风代码包已按 [`EASYINPUT_AUDIO_CAPTURE_V1_FROZEN`](../docs/contracts/easyinput-audio-capture-v1.md) 完成实现：固定参考 Maker `7619bd13f9ddfd6e2d80e2b8e022ef0acf32ce01` 的 LAN 音频合同先审计后清晰重实现，配置仅投影四个既有顶层字段，I2S0 使用 GPIO9/10/11 和现有 `KeyboardMic` 租约，采集/发送通过 64 帧 PSRAM 队列隔离并 fail-soft。Host 10/10、ESP-IDF 5.5.5 固定分区构建、桌面 127/127 与打包均通过；当前仍为 `HIL_NOT_AUTHORIZED`，未识别设备、未读取 Flash/NVS、未烧录。下一门是提交短分支后单独申请 app-only 真机收音验收；扬声器、BLE 和小智音频不进入本包。

- T08 双板正向 Link、小智重启恢复和逐根 TX/RX 断线已经真机通过，实体按键与旋钮也已回归；DeskMate 未运行时未执行的语音输入、打开应用、历史复制和配置读取继续列为 T09 联合回归项。T09 状态显示切片已冻结为 [`T09_AGENT_STATE_DISPLAY_V1_FROZEN`](../docs/contracts/t09-agent-state-display-v1.md)：桌面发送器、EasyInput 状态桥和小智 OLED 端已经在 `codex/t09-three-end-integration` 汇合，三套 Host/桌面测试与两套精确 ESP-IDF 构建通过。EasyInput `d961797` app 已完成恢复备份、app-only 写入、精确回读、正常 HID 启动以及新开发板 S1～S8/旋钮/灯效回归；小智 `b26e99e` app 也已完成恢复备份、app-only 写入、精确回读、正常应用启动与独立 OLED idle 大眼睛验收，静态不眨眼且舵机/音频无动作符合 T09 边界。当前状态为 `EASYINPUT_APP_FLASH_CONFIRMED / EASYINPUT_NORMAL_BOOT_CONFIRMED / EIGHT_KEY_ENCODER_LED_REGRESSION_CONFIRMED / XIAOZHI_APP_FLASH_CONFIRMED / XIAOZHI_NORMAL_BOOT_CONFIRMED / OLED_IDLE_CONFIRMED / THREE_END_STATE_HIL_PENDING`；下一步恢复三线并执行真实 VoiceWorkflow→OLED 闭环。模拟器和 mock 源始终不得写硬件。
