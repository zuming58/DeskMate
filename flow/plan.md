...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已进一步确认 T21 三段式实时对话、拍手/环境噪声抑制、真人中途插话、可信 Codex 活动数量和 T21G 专用本地关键词唤醒均已真实可用。当前 T21H 修复公放场景的延迟回声尾音：保留持续收音和真人插话，但把播放期间开始、播放结束后才返回的同一云端 ASR 片段隔离在模型、记忆和意图路由之前。自动回归与正式包已经通过，等待用户复测公放无话、途中插话和播放后立即说话。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21H three-stage companion, local wake and acoustic echo-tail guard | `codex/t21-three-stage-streaming-companion@0c54916fa14f86e4704ed7393505cdc18da26500` | User accepted realtime dialogue, human barge-in, clap/noise rejection, truthful Codex task count, voice grounding and T21G real keyword wake; T21H binds delayed post-drain ASR to the playback-contaminated item while accepting a distinct new utterance; Desktop `459/459`, package and native bridge self-test pass | `T21G_REAL_WAKE_HIL_ACCEPTED / T21H_CODE_BUILD_CONFIRMED / PUBLIC_SPEAKER_ECHO_AND_IMMEDIATE_NEW_SPEECH_HIL_PENDING / FIRMWARE_UNCHANGED` |
