...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已进一步确认 T21 三段式实时对话正常、拍手/环境噪声不会误打断、真人中途插话能够打断、Codex 活动任务数量可信且陪伴不再声称“没有麦克风”。稳定项目名播报已经进入当前实现；剩余拒收项是后台真人唤醒。T21F 的真实诊断证明 Windows 通用 `System.Speech` 只报告检测到声音，却始终不给出识别或拒绝结果，因此 T21G 已用随软件固定交付的专用中文关键词模型替换它，并直接复用 DeskMate 当前选择的 Windows 麦克风。自动回归与真实模型语音夹具已通过；用户回来后只需验收真人唤醒、二次恢复和房间噪声。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21G three-stage companion, stable project briefing and dedicated local wake | `codex/t21-three-stage-streaming-companion@22089b5861214b70cd6aa85eede7ce5d911736ff` | User accepted realtime dialogue, human barge-in, clap/noise rejection, truthful Codex task count and voice grounding; stable repository/project labels are implemented; T21G replaces rejected System.Speech wake with a pinned local open-vocabulary keyword model on the exact selected microphone; Desktop `457/457` and real upstream keyword fixture pass | `REALTIME_DIALOGUE_HUMAN_BARGE_NOISE_REJECTION_AND_TRUSTED_COUNT_HIL_ACCEPTED / T21G_CODE_AND_REAL_MODEL_SMOKE_CONFIRMED / PROJECT_LABEL_AND_REAL_WAKE_HIL_PENDING / FIRMWARE_UNCHANGED` |
