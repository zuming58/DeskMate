...
| --- | --- | --- | --- |
| T18 current software closure | `codex/t18-software-closure@274026b8edb1ceef03fcbd7807cf1cb2d35c195e` | Bridge/reporter health is observable and Desktop `394/394` previously passed, but user HIL proved task coverage incomplete and free-chat audio ownership unsafe | `WINDOWS_CODE_BUILD_CONFIRMED / USER_CONVERSATION_HIL_REJECTED / T16_REDESIGN_REQUIRED / FIRMWARE_UNCHANGED` |
...
10. **T16 desktop actions and Codex briefing — CODE/BUILD COMPLETE; USER HIL REJECTED; REDESIGN REQUIRED.** Direct launch remains restricted to explicitly voice-enabled registered applications. The current task reporter is bounded and private, but it is manual and does not identify actual Codex app tasks. The latest diagnostic proves the Bridge ran while a free-chat answer invented `65%`; the trusted contract contains no percentage. Contextual forms such as `什么任务/哪个任务` are not safely owned, and hiding provider text does not block its audio. Before another HIL: (a) all task-status-like uncertainty must fail closed; (b) no percentage may be spoken without trusted measured data; (c) provider free-chat audio must be prevented from racing a trusted turn; (d) an opt-in stable task identity/event adapter must populate the bounded store for every selected Codex task, including multiple projects. Until then the feature must say it has no trustworthy status rather than ask Doubao to infer one.
