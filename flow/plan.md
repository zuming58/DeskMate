...
## Current stage: T10D integrated software gates and Xiaozhi Stage 0 protocol HIL

目标：保留唯一 T10D 集成包的软件人工验收，同时保留已完成 app-only 烧录并通过状态往返验收的默认锁闭小智 T10D-C Stage 0。旧 T09.1 的 `UNKNOWN_TYPE` 已升级为真实的 locked/status 响应；实体舵机输出仍未测试，Stage 1 校准和 `MOTION` 能力继续锁闭。
...
7. **T10D-C real-adapter Stage 0 — PROTOCOL HIL CONFIRMED, SERVO OUTPUT UNTESTED.** Exact app-only flash/readback passed and the user-present panel completed the EasyInput -> DeskMate Link -> Xiaozhi `0x21` status round trip: owner is locked, selected axis is none and fixed step is 1°. The generic top-right unavailable indicator is treated as a possible stale/unrefreshed UI state until separately diagnosed, not as a failed terminal round trip. The default-off measured-profile gate still prevents PWM and keeps `MOTION` disabled. Stage 1 requires the documented user-present electrical/mechanical checklist plus a separately reviewed profile and authorization; presets, dancing and expression-linked motion wait until both axes have accepted centers, directions and limits.
