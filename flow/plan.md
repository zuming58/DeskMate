...
| --- | --- | --- | --- |
| T19 current software closure | `codex/t18-software-closure@20f893da77056293dca0c38fa384b7d65cfd5a0f` | Deterministic voice-enabled application matching; discoverable 网易云音乐 registration; encrypted local dance track; built-in/custom/voice dance music lifecycle; Desktop `417/417` and package passed | `WINDOWS_CODE_BUILD_CONFIRMED / USER_APP_AND_MUSIC_HIL_PENDING / FIRMWARE_UNCHANGED` |
...
14. **T12/T18 companion closure — LOCAL ENGINE CODE/SMOKE COMPLETE; USER HIL PENDING.** Regress persona versioning, provider pause, idle timeout and EasyInput call action. Windows `System.Speech` with an installed `zh-CN` recognizer is the opt-in local wake engine: it receives only the bounded configured phrases, outputs only ready/wake events, yields the microphone during dictation/realtime companion/mic test and resumes afterward. This machine passed a real two-second listen/release smoke; recognition quality and wake HIL remain user-observed gates.
