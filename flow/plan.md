...
10. **T16A desktop actions and Codex live briefing — T21D NOTIFICATION REPAIR CODED; REAL EVENT HIL PENDING.** Direct launch remains restricted to explicitly voice-enabled registered applications. Existing Codex lifecycle Hooks emit an opaque per-task identity into one bounded store; a separate read-only App Server catalog contributes only visible task titles and is never treated as live status. Aggregate questions report the active count and every bounded task line; unique names select one task; similar names request clarification. No model-authored percentage, ETA or progress fraction is accepted. Ordinary updates stay silent. Waiting/completed/error announce only on a real state edge, use one sentence, suppress repeated/late terminal events and close a notification-owned voice session immediately after playback. Contracts: [`t16a-codex-live-task-monitor-v1.md`](../docs/contracts/t16a-codex-live-task-monitor-v1.md), [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md).
...
14. **T20 companion entry/timing closure — T21D LOCAL-WAKE REPAIR CODED; USER PHRASE HIL PENDING.** Windows `System.Speech` with the installed `zh-CN` recognizer remains the explicitly enabled local background wake engine. Selected-microphone PCM now uses only the bounded exact grammar, generates joined/spaced cadence variants from the saved name and avoids unrestricted dictation competing for the same finite window. The capsule stays hidden until a hit; foreground audio owners still preempt wake and notification-owned speech resumes it immediately after closing. The diagnostic exporter now reads live main-process wake counters. Contract: [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md).
15. **T21 three-stage streaming companion — BASE DIALOGUE/NOISE REJECTION USER-ACCEPTED; T21D RETEST PENDING.** Bailian streaming ASR, one stateful DeskMate CompanionModel and sentence-streamed Doubao TTS remain active. Replies are no longer exposed in the capsule/renderer, and the model is grounded as a live voice participant instead of claiming it has no microphone. Explicit stop phrases remain prompt; ordinary cancellation now needs at least 500 ms of meaningful partial evidence or a five-character final backed by at least 650 ms of provider speech. Common non-speech labels, cumulative history, fillers, one fast hypothesis and close assistant echo cannot cancel playback. Verbose answers still close normally within the bounded speech queue. Contracts: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md), [`t21b-recognized-barge-in-and-local-wake-repair-v1.md`](../docs/contracts/t21b-recognized-barge-in-and-local-wake-repair-v1.md), [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md).
