...
- T11D Windows companion playback/stop hardening is implemented on `codex/t11d-companion-stop-drain-capsule`: provider `tts.end` now waits for a bounded, sequence-bound computer-speaker drain before releasing `working` and microphone suppression; explicit stop is idempotent and bounds every teardown dependency; the global live bar is a compact responsive capsule; and the desktop overview columns align while extra height expands the face stage. Contract: [`T11D_COMPANION_PLAYBACK_DRAIN_STOP_V1_FROZEN`](../docs/contracts/t11d-companion-playback-drain-stop-v1.md). User-present acceptance remains required.
