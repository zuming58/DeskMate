...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已进一步确认 T21 三段式实时对话、拍手/环境噪声抑制、真人中途插话、可信 Codex 活动数量和 T21G 专用本地关键词唤醒均已真实可用。T21H 已隔离公放场景的延迟回声尾音；本轮 T21I 继续修复一次真实插话卡死：插话已达到识别门槛并取消声音，但云端没有返回对应 final，旧状态机随后被普通空闲计时关闭。现在正常 final 优先，缺失时只把已通过门槛的稳定片段提升为一次最终句，短停止词恢复聆听，迟到同 item 结果不会重复。自动回归与正式包已经通过，等待用户重复插话验收。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21I three-stage companion, local wake and barge-final recovery | `codex/t21-three-stage-streaming-companion@963186d37d7f50bf269c42c55c473aeb8c05c7ed` | User accepted realtime dialogue, human barge-in, clap/noise rejection, truthful Codex task count, voice grounding and T21G real keyword wake; T21H rejects delayed public-speaker echo and T21I turns one accepted partial into exactly one turn when the provider omits final; Desktop `460/460`, package and native bridge self-test pass | `T21G_REAL_WAKE_HIL_ACCEPTED / T21H_ECHO_HIL_PENDING / T21I_CODE_BUILD_CONFIRMED / REPEATED_BARGE_LIVENESS_HIL_PENDING / FIRMWARE_UNCHANGED` |
