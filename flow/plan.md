...
- T08 双板正向 Link、小智重启恢复和逐根 TX/RX 断线已经真机通过，实体按键与旋钮也已回归；DeskMate 未运行时未执行的语音输入、打开应用、历史复制和配置读取继续列为 T09 联合回归项。T09 状态显示切片已冻结为 [`T09_AGENT_STATE_DISPLAY_V1_FROZEN`](../docs/contracts/t09-agent-state-display-v1.md)：桌面发送器、EasyInput 状态桥和小智 OLED 端已经在 `codex/t09-three-end-integration` 汇合，三套 Host/桌面测试与两套精确 ESP-IDF 构建通过。EasyInput `d961797` app 已完成恢复备份、app-only 写入和精确回读校验，当前状态为 `EASYINPUT_APP_FLASH_CONFIRMED / EASYINPUT_NORMAL_BOOT_PENDING / XIAOZHI_FLASH_PENDING`；下一步先恢复 EasyInput 正常启动并回归 USB/HID，再准备小智 app-only 烧录和真实 VoiceWorkflow→OLED 闭环。模拟器和 mock 源始终不得写硬件。
