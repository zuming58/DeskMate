...
8. **T10D-D user-present HIL — STAGE 2 FLASH VERIFIED / MOVEMENT ACCEPTANCE IN PROGRESS.** The explicitly authorized Stage 2 Xiaozhi app-only image was written and verified at `0x100000`; no bootloader, partition table, OTA data, NVS or eFuse was written. The exact integrated Windows package is running. The remaining gate is user observation of start/center, brief holds in four directions, release stop, return to center and emergency stop. Protocol terminal success remains separate from observed physical direction and motion.
