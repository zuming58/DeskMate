...
## Current stage: T15D adjustable physical choreography, T16 acceptance and T17 dual-source memory acceptance

目标：以已经通过用户真机验收的手动控制和四个固定动作为硬件基线，完成自定义舞蹈、默认舞蹈和左右/上下独立角度与速度设置的 Windows → EasyInput → 小智真实链路；同时保留 T16 白名单应用/Codex 简报和 T17 双来源记忆的独立验收。EasyInput T15D V2 已完成获授权的 app-only 烧录和独立回读校验，等待重新上电后的运行确认；小智 V2 仍必须单独取得明确授权后才可烧录。

### Current execution point

> **主 Agent 总控快照（2026-09-03，以下内容优先于本节后面的历史分支记录）**
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制和 T15 四个固定动作已完成用户真机验收；用户确认动作可用但希望上下幅度更明显、速度可调，并要求保存的自定义舞蹈可成为“跳舞”的真实内容。实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 已冻结并实现 T15D V2：Host HID 继续使用 `0x1A/0x1B`，Link 改用新增 `0x26/0x27`；设置页直接保存左右 `4..40°`、上下 `4..20°` 和两轴各 `20..100°/s`，小智依据原始云台限位和 Stage 2 脉宽包络再次限幅。内置默认舞蹈可见、可复制；只有明确激活的已保存舞蹈才会替换快速动作和语音“跳舞”。Windows 包和两套 app-only 候选均已构建并审计。EasyInput 精确镜像已获授权写入并独立回读一致，当前等待关机再开机确认运行；小智精确镜像尚未获本轮授权。软件窗口交付的舞蹈激活入口修复已审计并合入 `563c58b`，不改变任一固件或冻结合同；最终合并包已通过 `375/375` 和打包后输入桥自检。T16、T17 保持代码完成、用户验收待办。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@563c58b` | Fixed actions physically accepted; T15D V2 Windows `375/375`, native/package; activation UX integrated; EasyInput Host `15/15` + exact app flash/readback; Xiaozhi Host `16/16` + ESP-IDF v5.5.3 build | `T15_PRESET_HIL_ACCEPTED / EASYINPUT_V2_FLASH_VERIFIED_REBOOT_PENDING / XIAOZHI_V2_FLASH_AUTHORIZATION_PENDING` |
| T15D physical choreography | implementation `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c`, activation UX merged at `563c58b` | Frozen Host `0x1A/0x1B` V2 and Link `0x26/0x27`; 2–8 beats, three expressions, visible built-in dance, explicit always-visible custom activation, Yaw `4..40°`, Pitch `4..20°`, independent `20..100°/s`; EasyInput exact app written/read back, Xiaozhi exact app audited only | `V2_WIRE_FROZEN / WINDOWS_FINAL_PACKAGE_READY / EASYINPUT_FLASH_VERIFIED / XIAOZHI_FLASH_PENDING / HIL_PENDING` |
...
11. **T15D custom choreography V2 — EASYINPUT FLASH VERIFIED; XIAOZHI FLASH AND HIL PENDING.** The strict 2–8-beat editor sends one whole bounded program through Host `0x1A/0x1B` V2 and Link `0x26/0x27`; EasyInput only validates/forwards, while Xiaozhi owns target generation, per-axis speed, timing, display lease, center and stop. `设置与诊断 → 动作设置` exposes real semantic limits: Yaw `4..40°`, Pitch `4..20°`, and independent Yaw/Pitch `20..100°/s`. These values apply to fixed, custom and explicit voice actions and are clamped again on Xiaozhi. The built-in dance is a visible read-only selector entry; an explicitly activated saved choreography becomes semantic `dance` with its saved repeat count. Full Windows `375/375`, package/native self-test, EasyInput Host `15/15` + ESP-IDF v5.5.5 and Xiaozhi Host `16/16` + ESP-IDF v5.5.3 pass. EasyInput's exact app was backed up, written at `0x10000`, verified by esptool and independently read back to the authorized SHA-256 without touching any other partition. Next: power-cycle EasyInput, separately authorize/write the exact Xiaozhi app, then compare minimum/maximum angle and speed, execute the built-in dance, activate and execute one custom dance, and verify stop/center plus emergency stop. Contracts: [`easyinput-choreography-v2.md`](../contracts/deskmate-host/easyinput-choreography-v2.md), [`t15d-choreography-v2.md`](../contracts/deskmate-link/t15d-choreography-v2.md).
...
13. **T11E-B EasyInput local-speaker HIL — SEPARATE USER-AUTHORIZED GATE.** First audit the exact app-only image and preserved 16 MiB layout, then verify only the bounded low-volume startup probe and microphone-priority arbitration. It does not prove realtime speaker downlink, which remains `NOT_FROZEN`.
