...
## Current stage: T10D integrated software gates and Xiaozhi Stage 0 candidate

目标：保留唯一 T10D 集成包的软件人工验收，同时交付默认锁闭的小智 T10D-C Stage 0 固件候选，用于在单独授权后把旧 T09.1 的 `UNKNOWN_TYPE` 升级为真实的 locked/status 响应。实体舵机输出、Stage 1 校准和 `MOTION` 能力继续锁闭；本阶段不烧录、不驱动舵机。
...
| Xiaozhi T10D-C Stage 0 | `codex/xiaozhi-t10d-c-real-servo-adapter` from `a5a86b5e2ab816c2906d9f88a4ed4343dcd4db05` | Production T10C status owner + default-disabled real ESP-IDF adapter; Host `12/12`; exact ESP-IDF v5.5.3 fixed-layout build | `STAGE0_CODE_BUILD_COMPLETE / DEFAULT_LOCKED / STAGE1_PROFILE_BLOCKED / HIL_NOT_RUN` |
...
7. **T10D-C real-adapter Stage 0 — CODE/BUILD COMPLETE, HARDWARE LOCKED.** Xiaozhi now recognizes the frozen T10C status/command messages and contains a real GPIO11/GPIO12 50 Hz LEDC backend behind an explicit default-off measured-profile gate. The committed image cannot initialize PWM and keeps `MOTION` disabled. Stage 1 still requires the documented user-present electrical/mechanical checklist plus a separately reviewed profile and authorization; presets, dancing and expression-linked motion wait until both axes have accepted centers, directions and limits.
