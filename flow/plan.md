...
| --- | --- | --- | --- |
| T10D-D current integrated candidate | `codex/t10d-d-simplified-manual-control@95d9c95` | Restart recovery accepted on hardware: manual session opens, controls appear and Pitch physically moves; HIL found only the Windows up/down semantic sign reversed | `PHYSICAL_OUTPUT_CONFIRMED / PITCH_SEMANTIC_FIX_IN_PROGRESS` |
...
8. **T10D-D user-present HIL — ROUTE/OUTPUT CONFIRMED / PITCH UI SIGN FIX IN PROGRESS.** The fixed Windows package no longer returns `stale`: the manual session starts, centers and exposes the direction controls, and the user observed real Pitch movement. HIL also established this assembled unit's mechanical sign: current `up → pitch +1` moves down and `down → pitch -1` moves up. A Windows-only patch will exchange those semantic signs while leaving left/right, frozen wire bytes, firmware, centers, limits and step size unchanged. No firmware reflash is required. After the new package is merged and launched, rerun up/down, release, center and emergency-stop observations before freezing manual control and moving to motion presets.
