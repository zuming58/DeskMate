...
>
> T10D-D、T15/T15D V2 和本地关键词唤醒继续保持用户真机验收基线。当前 T21J 修复三段式陪伴的记忆连接：语音唤醒先简短回应；主进程保留最近 24 小时有界上下文，音频停止、重连和可信操作不清空，软件重启从已保存陪伴终句恢复；更早的近期片段和已审核长期记忆按问题检索。普通听写不进入陪伴回答链。记忆管理增加积压天数、按日期多批补整理和打开笔记目录；默认内部日期 Markdown，后续可选外部知识库，但不扫描其他资料。完整自动回归 472/472 已通过；真实招呼、续讲、长期回忆和笔记整理质量待用户验收。T21H/T21I 公放回声和重复插话仍保留稳定性回归门。现有两板固件冻结，不增加硬件、人物识别、其他 Agent 或音频下行。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21J three-stage dialogue continuity and reviewed retrieval | `codex/t21-three-stage-streaming-companion@d91268bc3b84bde77d24c4801faf6431e17f13c3` | Main-owned 24h context, cold restart from finals, interrupted-topic retention, owner-aware wake greeting, per-question reviewed vector retrieval and multi-day manual Markdown; Desktop `472/472`, Windows package and native self-test passed, exact package activated | `T21J_CODE_BUILD_CONFIRMED / EXACT_PACKAGE_RUNNING / GREETING_CONTEXT_RECALL_AND_DIGEST_HIL_PENDING / T21G_WAKE_HIL_ACCEPTED / FIRMWARE_UNCHANGED` |
