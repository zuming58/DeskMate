...
14. **T20 companion entry/timing closure — T21E EXACT LOCAL-WAKE RETEST PENDING.** Windows `System.Speech` with the installed `zh-CN` recognizer remains the explicitly enabled local background wake engine. Selected-microphone PCM uses only the bounded exact grammar, generates joined/spaced cadence variants from the saved name and avoids unrestricted dictation competing for the same finite window. A recognizer-accepted exact result now wakes without a second engine-specific absolute confidence cutoff; the unrestricted system-default compatibility fallback keeps its stricter gate. The capsule stays hidden until a hit; foreground audio owners still preempt wake and notification-owned speech resumes it immediately after closing. Diagnostics expose only content-free counts, including low-confidence compatibility matches. Contracts: [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md), [`t21e-stable-project-wake-v1.md`](../docs/contracts/t21e-stable-project-wake-v1.md).
15. **T21 three-stage streaming companion — DIALOGUE/BARGE/NOISE/TRUSTED COUNT USER-ACCEPTED; T21E RETEST PENDING.** Bailian streaming ASR, one stateful DeskMate CompanionModel and sentence-streamed Doubao TTS remain active. Replies are not exposed in the capsule/renderer, and the model is grounded as a live voice participant instead of claiming it has no microphone. Explicit stop phrases remain prompt; ordinary cancellation needs at least 500 ms of meaningful partial evidence or a five-character final backed by at least 650 ms of provider speech. The user has accepted ordinary dialogue,真人插话、拍手/噪声抑制、可信 Codex 活动数量和语音身份；稳定项目名播报与后台唤醒仍待本包真机复测。 Contracts: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md), [`t21b-recognized-barge-in-and-local-wake-repair-v1.md`](../docs/contracts/t21b-recognized-barge-in-and-local-wake-repair-v1.md), [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md), [`t21e-stable-project-wake-v1.md`](../docs/contracts/t21e-stable-project-wake-v1.md).
