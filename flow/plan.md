...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已进一步确认 T21 三段式实时对话正常、拍手/环境噪声不会误打断、真人中途插话能够打断、Codex 活动任务数量可信且陪伴不再声称“没有麦克风”。最新剩余问题只有 Windows 软件侧的项目身份和后台唤醒：任务总览误用了自动生成的线程标题；本地唤醒处理 891 个音频窗口并收到 2 个严格语法结果，却被 DeskMate 自己的固定 confidence 门槛全部拒绝。T21E 已改为稳定 Git 仓库/工作目录项目名并按项目聚合，同时让 `System.Speech` 已接受且与严格语法完全相等的结果直接唤醒；自动回归与 Windows 包通过，项目播报和真人唤醒待用户复测。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21E three-stage companion, stable project briefing and local wake | `codex/t21-three-stage-streaming-companion@04b508f037e05181ed8b74b136609cd667dd9651` | User accepted realtime dialogue, human barge-in, clap/noise rejection, truthful Codex task count and voice grounding; T21E replaces generated thread titles with stable repository/project labels and accepts an exact bounded-grammar result without an invalid second absolute confidence gate; Desktop `451/451`, package, catalog and InputBridge passed | `REALTIME_DIALOGUE_HUMAN_BARGE_NOISE_REJECTION_AND_TRUSTED_COUNT_HIL_ACCEPTED / T21E_CODE_BUILD_CONFIRMED / PROJECT_LABEL_AND_REAL_WAKE_RETEST_PENDING / FIRMWARE_UNCHANGED` |
...
| T17 dual-source memory | included in `codex/t15-t16-integration@242d7cc7279e992ed33101dd0ef4979a286b9b9b` | Shared review-first SQLite engine; standalone left navigation; separate `companion` / `dictation` source-day summaries; built-in prompt and exclusion rules; 23:30 schedule, next-start catch-up and automatic managed Markdown projection | `WINDOWS_CODE_BUILD_CONFIRMED / USER_HIL_PENDING / NO_FIRMWARE_CHANGE` |
...
12. **T17 dual-source memory — STANDALONE WINDOWS PAGE COMPLETE; USER HIL PENDING.** One SQLite/review/index/projection pipeline captures completed companion turns and successful real voice-input text as separate sources. Management is now a top-level left-navigation page immediately above diagnostics rather than an AI-companion subtab. The `逐句记录` filter reads the authoritative SQLite turns on demand with source/search/limit boundaries and hides internal session/event identifiers. The local model prompt is built in: summaries and candidates are source/day separated, sensitive/control/mock/failed material is excluded, only reviewed candidates enter long-term context and can be locally embedded. The default schedule remains 23:30 with next-start catch-up; configured managed Markdown projection remains automatic and retryable. This slice changes no firmware.
...
14. **T20 companion entry/timing closure — T21F CONTINUOUS LOCAL-WAKE HIL PENDING.** Windows `System.Speech` with the installed `zh-CN` recognizer remains the explicitly enabled local background wake engine. The system-default microphone now uses one continuous Windows recognizer rather than isolated finite windows; an explicitly selected microphone retains the bounded renderer-fed compatibility path. Exact bounded-grammar matches wake without a second absolute confidence cutoff, while unrestricted compatibility matching keeps its stricter gate. The capsule stays hidden until a hit; foreground audio owners still preempt wake and notification-owned speech resumes it immediately after closing. Diagnostics expose only content-free counts. Contracts: [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md), [`t21e-stable-project-wake-v1.md`](../docs/contracts/t21e-stable-project-wake-v1.md), [`t21f-continuous-background-wake-v1.md`](../docs/contracts/t21f-continuous-background-wake-v1.md).
15. **T21 three-stage streaming companion — DIALOGUE/BARGE/NOISE/TRUSTED COUNT USER-ACCEPTED; T21F WAKE RETEST PENDING.** Bailian streaming ASR, one stateful DeskMate CompanionModel and sentence-streamed Doubao TTS remain active. Replies are not exposed in the capsule/renderer, and the model is grounded as a live voice participant instead of claiming it has no microphone. Explicit stop phrases remain prompt; ordinary cancellation needs at least 500 ms of meaningful partial evidence or a five-character final backed by at least 650 ms of provider speech. The user has accepted ordinary dialogue,真人插话、拍手/噪声抑制、可信 Codex 活动数量和语音身份；稳定项目名播报与连续后台唤醒仍待本包真机复测。 Contracts: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md), [`t21b-recognized-barge-in-and-local-wake-repair-v1.md`](../docs/contracts/t21b-recognized-barge-in-and-local-wake-repair-v1.md), [`t21d-voice-notification-wake-v1.md`](../docs/contracts/t21d-voice-notification-wake-v1.md), [`t21e-stable-project-wake-v1.md`](../docs/contracts/t21e-stable-project-wake-v1.md), [`t21f-continuous-background-wake-v1.md`](../docs/contracts/t21f-continuous-background-wake-v1.md).
