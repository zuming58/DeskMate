...
- T10D-B Windows manual-calibration control is complete on `codex/t10d-desktop-manual-calibration-ui` from cumulative software base `35e627389282d8279d82646787f509681474c048`, consuming the frozen EasyInput host transport at `1645bf688b11d2f0d7ba3dfa7900f552886cb404`. It adds strict `0x16/0x17` codecs and native validation, one-request USB-epoch ownership, a status-before-action gate, four safety attestations, one-use ARM authorization, fixed one-degree yaw/pitch controls, emergency stop and separate user/EasyInput/Xiaozhi evidence. Current production `NOT_READY` remains truthful and disables actions. Full regression is `283/283`; Windows packaging passes. Contract: [`T10D_B_DESKTOP_MANUAL_CALIBRATION_V1_FROZEN`](../docs/contracts/t10d-b-desktop-manual-calibration-v1.md). T10D-C physical motion remains user-present and separately authorized.
...
- T12B.1 provider endpointing repair is built on `codex/t12b1-provider-endpointing-repair` from exact T12B base `bb0d29b4ce2ad7952fbac3bc6188593655107ab3`. User diagnostics excluded persistence and session-revision freeze, while the first live package proved that sending only `end_smooth_window_ms` left the provider on its short default. The current official API contract additionally defines `enable_custom_vad` as the default-false gate for custom stop detection. Commit `5977ad531e7e3f0c89b29cc79f968dde1b08c9c1` sends the bounded window plus that gate, preserves the accepted half-duplex `keep_alive` mode and excludes unrelated ASR fields. Full regression is `270/270`; Windows packaging passes. Exact-package eight-second utterance HIL remains open. Review: [`t12b1-provider-custom-vad-official-audit-2026-09-02.md`](../docs/reviews/t12b1-provider-custom-vad-official-audit-2026-09-02.md); handoff: [`t12b1-provider-custom-vad-repair-2026-09-02.md`](../docs/handoffs/t12b1-provider-custom-vad-repair-2026-09-02.md).

- T12B Companion layout/timing evidence is complete on `codex/t12b-companion-layout-timing-settings` from exact T12A base `7555d4161b90df000aae600a098ea336e198c743`. It removes the face-stretch layout chain, makes companion preferences an explicit validated write/readback transaction, expands manual pause/idle ranges, freezes one revision per new session and separates saved/session-applied values plus content-free partial-to-final and final-stop evidence. Full regression is `267/267` and Windows packaging passes. Contract: [`T12B_COMPANION_LAYOUT_TIMING_SETTINGS_V1_FROZEN`](../docs/contracts/t12b-companion-layout-timing-settings-v1.md); audit: [`t12b-t12a-hil-evidence-and-layout-audit-2026-09-01.md`](../docs/reviews/t12b-t12a-hil-evidence-and-layout-audit-2026-09-01.md); handoff: [`t12b-companion-layout-timing-settings-2026-09-01.md`](../docs/handoffs/t12b-companion-layout-timing-settings-2026-09-01.md). User-present visual/runtime confirmation remains open.

- T12A companion pause/identity/call is being implemented on `codex/t12a-companion-pause-identity-call` from accepted T11D.6 exact baseline `eb8fff8ac801eb0960e52282c22ac60b6efd9bc4`. T11D.6 HIL is now accepted: one connection, no reconnect/provider/transport/dialog error, 10 TTS turns with 8 completed and 2 explicitly abandoned, long response/automatic listen/manual interrupt all passed. This package adds provider-side 2/3/5-second pause control, an independent listening-only idle timeout, identity `小言`, a truthful unavailable wake boundary and reserved EasyInput `AI 陪伴呼唤` Host Action without firmware changes. Contract: [`T12A_COMPANION_PAUSE_IDENTITY_CALL_V1_FROZEN`](../docs/contracts/t12a-companion-pause-identity-call-v1.md); source audit: [`t12a-wake-word-and-endpointing-source-audit-2026-09-01.md`](../docs/reviews/t12a-wake-word-and-endpointing-source-audit-2026-09-01.md).

- T11D.6 Windows turn-ownership repair is implemented on `codex/t11d6-turn-cancel-diagnostics` from exact T11D.5 base `2169c2575a79921f7286d48283db144b71a9c3c5`. T11D.5 fixed the provider idle timeout, but its handler-lag-dependent half-duplex gate still allowed PCM/ASR during `thinking` and could classify ASR behind an already-arrived TTS event as a user turn; every accepted ASR final then unconditionally interrupted local playback. T11D.6 captures a synchronous closed arrival phase before event queueing, permits uplink only in `listening`, keeps `thinking/speaking/draining` closed, removes normal-ASR cancellation, and records content-free turn completion/abandonment and sink cancellation causes. Contract: [`T11D6_TURN_CANCEL_DIAGNOSTICS_V1_FROZEN`](../docs/contracts/t11d6-turn-cancel-diagnostics-v1.md); review: [`t11d6-intermediate-answer-truncation-root-cause-2026-09-01.md`](../docs/reviews/t11d6-intermediate-answer-truncation-root-cause-2026-09-01.md).

- T11D.5 Windows half-duplex keep-alive repair is implemented on `codex/t11d5-dialog-error-root-ui`. Exact T11D.4 HIL showed complete playback, zero local queue/drain/transport failure and 600 intentional upstream drops before an adjacent dialog error. Current official documentation requires `dialog.extra.input_mod=keep_alive` when microphone upload may pause and defines the matching `DialogAudioIdleTimeoutError`; the session now declares that mode while preserving strict echo suppression, same-session event `359` and fail-closed event `599`. The companion restart/actions and source cards are also in one responsive normal-flow stack. Contract: [`T11D5_HALF_DUPLEX_KEEPALIVE_V1_FROZEN`](../docs/contracts/t11d5-half-duplex-keepalive-v1.md); review: [`t11d5-dialog-audio-idle-timeout-root-cause-2026-09-01.md`](../docs/reviews/t11d5-dialog-audio-idle-timeout-root-cause-2026-09-01.md).
