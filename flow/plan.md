...
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T15/T16 三端代码、构建和双板烧录均已完成；首次实体预设验收暴露的 Windows `0x18/0x19` 集合误选已经修复，主 Agent 用最终原生桥完成了只读真机状态往返，确认 `FF00:0009`、EasyInput 和小智动作端均可达。T15D 紧凑编排界面与 T17 双来源记忆也已合入总控分支。当前不再需要烧录，下一道门是用户在最终软件包中验收四个实体预设和两类记忆流水线；自定义舞蹈的实体传输仍保持关闭。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@1fb27cb` | Desktop `373/373` + Windows package/native self-test; final bridge read-only HIL reaches `FF00:0009` and returns the Xiaozhi endpoint with adapter available, no fault and a truthful recenter-required state; both firmware trees are unchanged from the already-flashed baseline | `WINDOWS_FIX_MERGED / DUAL_BOARD_FLASH_COMPLETE / PRESET_PHYSICAL_HIL_READY` |
...
| T17 dual-source memory | integrated in `codex/t15-t16-integration@1fb27cb` | Shared review-first SQLite engine; separate `companion` / `dictation` source-day summaries; 23:30 default schedule and next-start catch-up; automatic managed Markdown projection plus visible retry | `WINDOWS_CODE_BUILD_CONFIRMED / USER_HIL_PENDING / NO_FIRMWARE_CHANGE` |
...
9. **T15 runtime preset motion — DUAL FLASH AND WINDOWS REPAIR COMPLETE; PHYSICAL HIL READY.** Additive HID `0x18/0x19` and DeskMate Link `0x22/0x23` are frozen and implemented. The native bridge now selects the descriptor's dedicated `FF00:0009` collection while manual `FF00:0007` and configuration `FF00:0002` remain unchanged. A final read-only status request reached the flashed Xiaozhi endpoint and returned adapter available, no fault, no emergency latch and recenter required; this is the expected state before the first action automatically centers. Next run attention → nod twice → search → dance twice → stop/center and emergency-stop/recovery. Voice and automatic motion remain disabled until the four presets pass.
...
12. **T17 dual-source memory — WINDOWS CODE/BUILD COMPLETE; USER HIL PENDING.** One SQLite/review/index/projection pipeline now captures completed companion turns and successful real voice-input text as separate sources. Each source has independent source-day summaries, candidates, filters and last-run state; the default local schedule is 23:30 with next-start catch-up. A successful scheduled or manual digest automatically projects the authoritative snapshot under `DeskMate/daily/<source>/` when the user has selected a knowledge-base folder; projection failures preserve SQLite and surface a retryable warning. Only reviewed candidates enter long-term context, and this slice changes no firmware.
