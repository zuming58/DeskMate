...
目标：以已经通过用户真机验收的手动控制和四个固定动作为硬件基线，完成自定义舞蹈、默认舞蹈和左右/上下独立角度与速度设置的 Windows → EasyInput → 小智真实链路；同时保留 T16 白名单应用/Codex 简报和 T17 双来源记忆的独立验收。EasyInput 与小智 T15D V2 均已完成获授权的 app-only 烧录和独立回读校验；当前 HIL 阻塞在 Windows 原生输入桥仍按 V1 校验 `0x1A/0x1B`，不是固件缺失。

### Current execution point

> **主 Agent 总控快照（2026-09-03，以下内容优先于本节后面的历史分支记录）**
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制和 T15 四个固定动作已完成用户真机验收；T15D V2 实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 冻结 Host `0x1A/0x1B` 与 Link `0x26/0x27`，提供 Yaw `4..40°`、Pitch `4..20°` 及两轴 `20..100°/s`。EasyInput 与小智精确 V2 app-only 镜像都已获授权写入并独立回读一致。最新 HIL 已否决当前 Windows 包：Electron、EasyInput 和小智均实现 V2，但 `native/DeskMate.InputBridge/VendorReportProtocol.cs` 仍只接受 V1 版本、布局、CRC、`0x24/0x25` 与 `1..3` profile，导致 V2 在电脑端被拒绝；快速点头静默回退到旧版 `+6/-2°`，自定义舞蹈则无回退而不动。原始小智参考代码复核证明真实点头采用 Pitch `±20°`，与产品 Stage 2/V2 限位和方向一致，因此本轮无需再烧固件。下一步先修复并用冻结 V2 黄金向量覆盖原生桥，再重打 Windows 包进行最大 Pitch 与自定义舞蹈 HIL。T16、T17 保持代码完成、用户验收待办。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@2ba9f9d` | Fixed actions physically accepted; both exact T15D V2 apps flashed/read back; latest Windows package launched; HIL diagnostic captured | `T15_PRESET_HIL_ACCEPTED / DUAL_V2_FIRMWARE_FLASH_VERIFIED / WINDOWS_NATIVE_V2_REPAIR_REQUIRED` |
| T15D physical choreography | implementation `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c`, activation UX merged at `563c58b` | Frozen Host `0x1A/0x1B` V2 and Link `0x26/0x27`; Electron and both firmware ends agree; native input bridge remains V1-only and rejects V2 before EasyInput; reference nod is Pitch `±20°` | `V2_WIRE_FROZEN / DUAL_FIRMWARE_FLASH_VERIFIED / HIL_FAILED / WINDOWS_NATIVE_BRIDGE_ROOT_CAUSE_CONFIRMED / NO_REFLASH_REQUIRED` |
...
11. **T15D custom choreography V2 — DUAL FIRMWARE FLASH VERIFIED; WINDOWS NATIVE BRIDGE REPAIR REQUIRED.** The strict 2–8-beat editor emits one whole bounded program through Host `0x1A/0x1B` V2 and Link `0x26/0x27`; both firmware endpoints implement and validate the frozen V2 fields, and both exact app images have been written/read back. Current HIL exposed a Windows-only cross-language skew: Electron emits V2, but `VendorReportProtocol.cs` still validates V1 offsets and therefore blocks V2 before EasyInput and discards valid V2 responses. Legacy quick-action fallback explains the observed old small nod; custom actions have no fallback. The original Xiaozhi reference uses a 90-degree center with Pitch `±20°`, matching the product Stage 2 envelope and V2 target signs. Next: add native V1+V2 validation using the exact frozen golden vectors, rebuild the package, then compare maximum Pitch and speed, execute the built-in dance, activate/execute one custom dance, and verify stop/center plus emergency stop. No firmware reflash is required. Contracts: [`easyinput-choreography-v2.md`](../contracts/deskmate-host/easyinput-choreography-v2.md), [`t15d-choreography-v2.md`](../contracts/deskmate-link/t15d-choreography-v2.md).
