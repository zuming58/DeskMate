...
## Current stage: T10D integrated software gates and Xiaozhi Stage 1 reference-baseline micro-trial

目标：保留唯一 T10D 集成包的软件人工验收，同时从已通过状态往返验收的默认锁闭小智 T10D-C Stage 0，推进到一个可审计、单轴、中心附近仅 ±1° 的参考基线真机候选。实体舵机输出仍未测试；生产 `MOTION`、预设动作、跳舞和表情联动继续锁闭。
...
| T14A Windows follow-on | `codex/t14-desktop-agent-adapter-framework@8578f0cc8bef40ba269bb0960adbaf04c66432ed`, based on integrated Flow HEAD `1f7b58e60b288ebd8d3a65caa71fb926a69ff3ee` | Codex-compatible generic provider status plus strict Hermes lifecycle adapter; Desktop `289/289` + package | `SOFTWARE_FOLLOW_ON / CODE_BUILD_CONFIRMED / HIL_NOT_RUN / NOT_MERGED` |
...
| Xiaozhi Stage 1 reference trial | `codex/xiaozhi-t10d-c-reference-baseline-trial` | Exact fixed-reference GPIO/50 Hz/1500 us conversion plus the operator's prior same-unit nod/rotation evidence; 1489..1511 us single-axis envelope; Host `12/12`; ESP-IDF v5.5.3 separate-config fixed-layout build | `CODE_BUILD_CONFIRMED / FLASH_NOT_AUTHORIZED / MOTION_NOT_RUN` |
...
6. **T14A Hermes Agent adapter — CODE/BUILD COMPLETE, NOT MERGED.** The software task delivered `codex/t14-desktop-agent-adapter-framework@8578f0cc8bef40ba269bb0960adbaf04c66432ed` from the exact integrated Flow HEAD. It preserves Codex, adds one strict local Hermes lifecycle adapter and packages an optional template without installing or enabling it. Desktop `289/289` and package pass; real Hermes enablement/OLED observation remains user-present HIL. Keep it as a follow-on until the current acceptance point is selected; do not silently replace the exact T10D package under test.
7. **T10D-C Stage 1 reference-baseline micro-trial — CODE/BUILD COMPLETE, FLASH AUTHORIZATION NEXT.** Stage 0 app-only flash/readback and the user-present `0x21` locked/status round trip passed. The operator confirmed this same assembled unit previously nodded and rotated normally with the fixed reference firmware, so the next candidate reuses its exact GPIO11/GPIO12, 50 Hz and 1500 us center conversion while narrowing each axis to 1489..1511 us. It permits only status-first, one selected axis, four attestations, one-use ARM, provisional center, one ±1° observation, recenter and emergency stop. Default configuration remains Stage 0; no device access, flash or physical motion has occurred for Stage 1. Identify the exact Xiaozhi endpoint, obtain explicit app-only authorization, then test one axis at a time. Presets, dancing, expression-linked motion and production `MOTION` remain later gates.
8. **T11E-B EasyInput local-speaker HIL — SEPARATE USER-AUTHORIZED GATE.** First audit the exact app-only image and preserved 16 MiB layout, then verify only the bounded low-volume startup probe and microphone-priority arbitration. It does not prove realtime speaker downlink, which remains `NOT_FROZEN`.
