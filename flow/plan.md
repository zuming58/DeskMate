...
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制和 T15 四个固定动作已完成用户真机验收；用户确认动作可用但希望上下幅度更明显、速度可调，并要求保存的自定义舞蹈可成为“跳舞”的真实内容。实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 已冻结并实现 T15D V2：Host HID 继续使用 `0x1A/0x1B`，Link 改用新增 `0x26/0x27`；设置页直接保存左右 `4..40°`、上下 `4..20°` 和两轴各 `20..100°/s`，小智依据原始云台限位和 Stage 2 脉宽包络再次限幅。内置默认舞蹈可见、可复制；只有明确激活的已保存舞蹈才会替换快速动作和语音“跳舞”。Windows 包和两套 app-only 候选均已构建并审计。EasyInput 精确镜像已获授权写入并独立回读一致，当前等待关机再开机确认运行；小智精确镜像尚未获本轮授权。T16、T17 保持代码完成、用户验收待办。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` | Fixed actions physically accepted; T15D V2 Windows `375/375`, native/package; EasyInput Host `15/15` + exact app flash/readback; Xiaozhi Host `16/16` + ESP-IDF v5.5.3 build | `T15_PRESET_HIL_ACCEPTED / EASYINPUT_V2_FLASH_VERIFIED_REBOOT_PENDING / XIAOZHI_V2_FLASH_AUTHORIZATION_PENDING` |
| T15D physical choreography | integrated at `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` | Frozen Host `0x1A/0x1B` V2 and Link `0x26/0x27`; 2–8 beats, three expressions, visible built-in dance, explicit custom activation, Yaw `4..40°`, Pitch `4..20°`, independent `20..100°/s`; EasyInput exact app written/read back, Xiaozhi exact app audited only | `V2_WIRE_FROZEN / EASYINPUT_FLASH_VERIFIED / XIAOZHI_FLASH_PENDING / HIL_PENDING` |
