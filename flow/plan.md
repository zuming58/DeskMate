...
| --- | --- | --- | --- |
| T10D-D current integrated candidate | `codex/t10d-d-simplified-manual-control@20a241b` | Direction semantics accepted on hardware; emergency stop latches correctly, but simplified Windows flow lacks an explicit clear-and-restart path | `MOTION_HIL_PARTIAL_ACCEPTED / E_STOP_RECOVERY_FIX_IN_PROGRESS` |
...
8. **T10D-D user-present HIL — DIRECTIONS ACCEPTED / E-STOP RECOVERY FIX IN PROGRESS.** The Pitch-fixed package is accepted for directional movement, and immediate stop correctly latches the Xiaozhi owner. HIL found one Windows orchestration omission: after the latch, the simplified page has no explicit `clearEmergencyStop` recovery path, so a later start is rejected as `emergency-stopped`. A Windows-only repair will relabel the primary action and, only after explicit environment confirmation/click, perform status → clear latch → verify locked → rebuild both centers. Startup/reconnect never auto-clear. No firmware reflash is required. After rerun of clear/restart, release, center and stop, freeze manual control and proceed to bounded preset movements.
