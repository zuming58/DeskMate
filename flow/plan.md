...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已确认 T21 三段式实时对话、语音动作和噪声不再误打断，但最新真机回归发现普通真人语音仍可能被过严的“两次前缀一致”门槛拒绝，随后过长回答会把会话带入错误态。T21C 已按 Qwen 的 confirmed `text` / draft `stash` 语义改为“确认前缀、两次不同有效草稿或时长支撑 final”三条人声证据路径，并把过长回答改成有界收尾、正常回聆听。自动回归和 Windows 包已通过，普通真人插话、扬声器回声、长回答恢复与后台真人唤醒仍待用户验收。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21C three-stage companion, evidence-gated barge-in and local wake repair | `codex/t21-three-stage-streaming-companion@b0e7365` | User confirmed realtime conversation, voice motion and noise rejection; human-barge/long-answer HIL rejected the T21B candidate; T21C preserves provider-confirmed text, accepts sparse valid speech and bounds verbose answers without terminating the session; Desktop `448/448`, package and InputBridge passed | `BASE_DIALOGUE_AND_NOISE_REJECTION_HIL_ACCEPTED / HUMAN_BARGE_HIL_REJECTED / T21C_CODE_BUILD_CONFIRMED / SYNTHETIC_WAKE_SMOKE_PASSED / USER_SPEECH_ECHO_LONG_REPLY_AND_WAKE_RETEST_PENDING / FIRMWARE_UNCHANGED` |
...
15. **T21 three-stage streaming companion — BASE DIALOGUE/NOISE REJECTION USER-ACCEPTED; T21C HUMAN-BARGE RETEST PENDING.** Bailian streaming ASR, one stateful DeskMate CompanionModel and sentence-streamed Doubao TTS are active. ASR remains open for the whole audible playback window and evaluates only the current provider item at balanced VAD threshold `0.2`. Explicit interruption needs a matching server speech start; ordinary speech may qualify through a provider-confirmed prefix, two distinct meaningful draft hypotheses, or a duration-backed final. Cumulative history, fillers, one unconfirmed hypothesis and close assistant echo cannot cancel playback. Verbose answers are capped and close with a bounded spoken sentence instead of failing the session. Background wake uses bounded finite PCM windows and passes the generated 16 kHz phrase smoke; human wake, speaker-echo immunity, ordinary-sentence barge-in and long-answer recovery remain the user gate. Contracts: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md), [`t21b-recognized-barge-in-and-local-wake-repair-v1.md`](../docs/contracts/t21b-recognized-barge-in-and-local-wake-repair-v1.md).
