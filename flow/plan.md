...
## Current stage: T22B scene rail and a single keyboard-diagram editor
...
- 自动证据：完整测试 500/500；原生 Electron 交互 44 项、1440×1024 与 960×680 布局通过。桌面包和实际启动证据见最新 progress，不以单测代替硬件验收。
- 当前待人工：先看新版按键页面的布局和单一编辑流程，不要求立刻做功能测试。后续再明确预览/确认配置，验收真实旋钮、焦点返回/粘贴、KEY5～7 场景切换、豆包场景播报及语音入口；推荐方案只在用户选择后填入本机。
