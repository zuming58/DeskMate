...
## Current stage: T15 physical-preset acceptance, T16 acceptance and T17 dual-source memory acceptance
...
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T15/T16 三端代码、构建和双板烧录均已完成；实体预设首次用户测试又暴露一个 Windows 恢复优先级缺陷：小智同时报告 `state=not-ready` 与 `result=recenter-required` 时，软件在发送自动回中前错误退出。`37ba8e3` 已用用户导出的真实诊断向量修复并通过完整回归。T17 记忆管理现为左侧独立入口，统一管理陪伴与语音输入；整理提示词维持内置。当前不需要重新烧录，下一道门仍是用户用最终软件包重验四个实体预设和两类记忆流水线；自定义舞蹈的实体传输保持关闭。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@37ba8e3984d538662dbff8f58f132fec36da3433` | User diagnostic proves `FF00:0009`, EasyInput and Xiaozhi status connected; exact `not-ready + recenter-required` recovery vector now automatically centers before RUN; focused `49/49`, Desktop `373/373`, Windows package/native self-test; both firmware trees unchanged | `WINDOWS_RECENTER_PRECEDENCE_FIXED / DUAL_BOARD_FLASH_COMPLETE / PRESET_PHYSICAL_HIL_RETEST_READY` |
| T15D Windows editor | `codex/t15d-desktop-choreography-editor@2d7e0c0111e36d3cb0eca63c91670d0e069385d3`, merged as `a3b6ae2` | Strict 2–8 beat Yaw/Pitch/Expression editor, local store and software preview; focused `14/14`, full `359/359`, package/native self-test and 1440x1024 compact visual QA | `MERGED / WINDOWS_CODE_BUILD_CONFIRMED / ENTITY_EXECUTION_FAIL_CLOSED / WIRE_NOT_FROZEN` |
...
| T17 dual-source memory | integrated in `codex/t15-t16-integration@37ba8e3984d538662dbff8f58f132fec36da3433` | Shared review-first SQLite engine; standalone left navigation; separate `companion` / `dictation` source-day summaries; built-in prompt and exclusion rules; 23:30 schedule, next-start catch-up and automatic managed Markdown projection | `WINDOWS_CODE_BUILD_CONFIRMED / USER_HIL_PENDING / NO_FIRMWARE_CHANGE` |
...
9. **T15 runtime preset motion — DUAL FLASH AND WINDOWS RECENTER REPAIR COMPLETE; PHYSICAL HIL RETEST READY.** Additive HID `0x18/0x19` and DeskMate Link `0x22/0x23` are frozen and implemented. The user-exported diagnostic proves the dedicated `FF00:0009` collection and Xiaozhi adapter are available, but also exposed the valid startup combination `state=not-ready` plus `result=recenter-required`. Windows now gives the specific recovery result precedence, sends one automatic stop-and-center, waits for READY and only then sends RUN. The exact real vector is covered by regression. Next run attention → nod twice → search → dance twice → stop/center and emergency-stop/recovery. Voice and automatic motion remain disabled until the four presets pass.
...
11. **T15D custom choreography — DIRECT-CONTROL WINDOWS UX COMPLETE; WIRE NOT FROZEN.** The strict 2–8-beat store/compiler remains. Each beat now uses direct left/center/right and up/center/down controls, three visible expressions and blank-as-hold; the duplicated large face is removed and all actions share one compact bar below the grid. Software preview is available, while entity execution remains visibly disabled behind a fail-closed pending adapter. New Host/Link IDs and both firmware endpoints stay blocked on the current four-preset physical HIL. Design: [`t15d-choreography-editor-v1.md`](../docs/design/t15d-choreography-editor-v1.md).
