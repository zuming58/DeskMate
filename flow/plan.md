...
## Current stage: T15D physical choreography update, T16 acceptance and T17 dual-source memory acceptance

目标：以已经通过用户真机验收的手动控制和四个固定动作作为硬件基线，完成自定义舞蹈、默认舞蹈和统一动作强度/速度设置的 Windows → EasyInput → 小智真实链路；同时保留 T16 白名单应用/Codex 简报和 T17 双来源记忆的独立验收。两套 T15D 固件候选已经构建，但必须由用户逐板明确授权后才可 app-only 烧录。

### Current execution point

> **主 Agent 总控快照（2026-09-03，以下内容优先于本节后面的历史分支记录）**
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制和 T15 四个固定动作已完成用户真机验收；用户确认动作可用但希望幅度更大、速度更快，并要求保存的自定义舞蹈可成为“跳舞”的真实内容。`242d7cc7279e992ed33101dd0ef4979a286b9b9b` 已冻结并实现 T15D HID `0x1A/0x1B`、Link `0x24/0x25`、小智本地节拍/表情调度、EasyInput 单请求转发、默认舞蹈及设置页三档强度/速度。Windows 包和两套 app-only 候选均构建通过；本轮没有接触硬件，下一道门是分别授权烧录 EasyInput 与小智，再做实体编舞与档位真机验收。T16、T17 保持代码完成、用户验收待办。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@242d7cc7279e992ed33101dd0ef4979a286b9b9b` | Fixed actions physically accepted; T15D Windows `375/375`, native/package; EasyInput Host `15/15` + ESP-IDF v5.5.5; Xiaozhi Host `16/16` + ESP-IDF v5.5.3 | `T15_PRESET_HIL_ACCEPTED / T15D_THREE_END_CODE_BUILD_CONFIRMED / DUAL_FIRMWARE_FLASH_AUTHORIZATION_PENDING` |
| T15D physical choreography | integrated at `242d7cc7279e992ed33101dd0ef4979a286b9b9b` | Frozen Host `0x1A/0x1B`, Link `0x24/0x25`; 2–8 beats, three expressions, default dance and bounded strength/tempo profiles; both exact app-only candidates audited | `WIRE_FROZEN / WINDOWS_AND_DUAL_FIRMWARE_BUILD_CONFIRMED / HIL_PENDING` |
...
11. **T15D custom choreography — THREE-END CODE/BUILD COMPLETE; DUAL FLASH AND HIL PENDING.** The strict 2–8-beat editor now sends one whole bounded program through Host `0x1A/0x1B` and Link `0x24/0x25`; EasyInput only validates/forwards, while Xiaozhi owns poses, timing, display lease, center and stop. A saved action may be marked as the default semantic `dance`. `设置与诊断 → 动作设置` exposes only `柔和/标准/明显` and `舒缓/标准/利落`, applied to fixed, custom and explicit voice actions. Full Windows `375/375`, package/native self-test, EasyInput Host `15/15` + ESP-IDF v5.5.5 and Xiaozhi Host `16/16` + ESP-IDF v5.5.3 pass. Next: separately authorize both app-only images, then test strength/tempo, one custom dance, default “跳舞”, stop/center and emergency stop. Contracts: [`easyinput-choreography-v1.md`](../contracts/deskmate-host/easyinput-choreography-v1.md), [`t15d-choreography-v1.md`](../contracts/deskmate-link/t15d-choreography-v1.md).
