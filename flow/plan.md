...
| T10D-D simplified control | `codex/t10d-d-simplified-manual-control` | Existing wire retained; Windows orchestration frozen; Stage 2 fixed-reference envelope overlay; Xiaozhi Host `12/12` + ESP-IDF v5.5.3 | `SOFTWARE_IN_PROGRESS / STAGE2_CODE_BUILD_CONFIRMED / HIL_NOT_RUN` |
...
7. **T10D-C Stage 1 route trial — FLASH/PROTOCOL HIL COMPLETE, OUTPUT NOT RUN.** The exact app-only image was written and verified. Status and yaw selection reached Xiaozhi. The attempted step was rejected as `CENTER_REQUIRED` with `completed_output_count=0`, so no PWM/output occurred; this identifies missing center orchestration rather than a servo failure.
8. **T10D-D simplified press-and-hold control — IN PROGRESS.** Windows owns the simple interaction and expands it into the unchanged frozen operations with one terminal-gated request at a time. Xiaozhi gets only a separate Stage 2 profile overlay restoring the fixed-reference yaw/pitch ranges already exercised by this assembled unit. EasyInput firmware does not change. After software tests/package and Xiaozhi Host/IDF build pass, request one new app-only authorization and run user-present start/hold/release/center/e-stop acceptance.
