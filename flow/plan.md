...
| --- | --- | --- | --- |
| T10D-D current integrated candidate | `codex/t10d-d-simplified-manual-control@2a8c4e7` | Desktop request-ID restart recovery merged; Desktop `316/316` + exact Windows package; EasyInput Stage 2-compatible firmware and Xiaozhi Stage 2 app are already flashed/verified | `WINDOWS_FIXED / EXACT_PACKAGE_RUNNING / PHYSICAL_OUTPUT_NOT_RUN` |
...
8. **T10D-D user-present HIL — STAGE 2 FLASH VERIFIED / FIXED WINDOWS PACKAGE RUNNING.** The Windows restart regression is fixed by a persisted high-water request-ID sequence and bounded status-only stale recovery; the frozen HID/DeskMate Link bytes and both firmware images are unchanged. The repair is merged into the integration branch, Desktop regression passes `316/316`, and the exact recovery package is running after all old-package processes were closed. No firmware reflash is required. Next user action is one environment confirmation and one start press; only after an accepted fresh status/center sequence should the four-direction controls appear. Protocol terminal success remains separate from observed physical direction and motion.
