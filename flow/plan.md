...
目标：以已经通过用户真机验收的手动控制和四个固定动作为硬件基线，完成自定义舞蹈、默认舞蹈和左右/上下独立角度与速度设置的 Windows → EasyInput → 小智真实链路；同时保留 T16 白名单应用/Codex 简报和 T17 双来源记忆的独立验收。EasyInput 与小智 T15D V2 均已完成获授权的 app-only 烧录和独立回读校验；Windows 原生输入桥的 V1/V2 版本偏差已经修复、集成并重打包，当前只剩最大 Pitch 点头与自定义舞蹈的用户真机复验。

### Current execution point

> **主 Agent 总控快照（2026-09-03，以下内容优先于本节后面的历史分支记录）**
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制和 T15 四个固定动作已完成用户真机验收；T15D V2 实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 冻结 Host `0x1A/0x1B` 与 Link `0x26/0x27`，提供 Yaw `4..40°`、Pitch `4..20°` 及两轴 `20..100°/s`。EasyInput 与小智精确 V2 app-only 镜像都已获授权写入并独立回读一致。Windows 原生桥的 V1-only 根因已由 `0bf131dacb73c0dd6c8d420b4620de1ae3ffe110` 修复：真实原生自检覆盖冻结 V1/V2 黄金向量和版本、CRC、Link、数字范围、padding 失败向量，Electron 诊断明确区分 `v2-success` 与 `legacy-fallback`。主 Agent 已集成、通过 Desktop `378/378`、重新打包并验证包内原生自检；固件树零变化，不需要再次烧录。下一步只运行最大 Pitch 点头和一个当前自定义舞蹈 HIL，必须取得终态 `v2-success`。T16、T17 保持代码完成、用户验收待办。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration`, Windows V2 repair merged through `0120f1acd585d71f0d7983ba00147cf6637105f5` | Fixed actions physically accepted; both exact T15D V2 apps flashed/read back; native V1/V2 bridge repair independently reviewed, full-tested and repackaged | `T15_PRESET_HIL_ACCEPTED / DUAL_V2_FIRMWARE_FLASH_VERIFIED / WINDOWS_NATIVE_V2_REPAIRED / HIL_PENDING` |
| T15D physical choreography | implementation `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c`, activation UX merged at `563c58b`, native repair `0bf131dacb73c0dd6c8d420b4620de1ae3ffe110` | Frozen Host `0x1A/0x1B` V2 and Link `0x26/0x27`; Electron, native bridge and both firmware ends now agree; native self-test consumes the exact V1/V2 golden vectors; reference nod is Pitch `±20°` | `V2_WIRE_FROZEN / DUAL_FIRMWARE_FLASH_VERIFIED / WINDOWS_PACKAGE_READY / MAX_PITCH_AND_CUSTOM_HIL_PENDING / NO_REFLASH_REQUIRED` |
...
11. **T15D custom choreography V2 — DUAL FIRMWARE FLASH VERIFIED; REPAIRED WINDOWS PACKAGE READY FOR HIL.** The strict 2–8-beat editor emits one whole bounded program through Host `0x1A/0x1B` V2 and Link `0x26/0x27`; both firmware endpoints implement and validate the frozen V2 fields, and both exact app images have been written/read back. The Windows cross-language skew is repaired: `VendorReportProtocol.cs` now validates V1 rollback and V2 separately, its real self-test consumes both frozen golden-vector sets and isolated corruptions, Electron correlates protocol version, and diagnostic output distinguishes `v2-success`, `native-rejected`, `v2-failed` and `legacy-fallback`. The rebuilt package passes Desktop `378/378` and packaged native self-test. Next: compare maximum Pitch and speed, execute the built-in dance, activate/execute one custom dance, and verify stop/center plus emergency stop; only terminal `v2-success` closes HIL. No firmware reflash is required. Contracts: [`easyinput-choreography-v2.md`](../contracts/deskmate-host/easyinput-choreography-v2.md), [`t15d-choreography-v2.md`](../contracts/deskmate-link/t15d-choreography-v2.md).
