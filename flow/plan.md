...
8. **T10D-D user-present HIL — STAGE 2 FLASH VERIFIED / WINDOWS STALE-ID FIX IN PROGRESS.** The explicitly authorized Stage 2 Xiaozhi app-only image was written and verified at `0x100000`; no bootloader, partition table, OTA data, NVS or eFuse was written. The first exact-package start attempt was rejected by EasyInput as `stale` before accepted/terminal/output because the restarted Windows process reused low request IDs within the same USB mount epoch. A Windows-only monotonic-ID repair is active in `DeskMate软件开发`; no firmware reflash is required. After merge/repackage, resume start/center, brief holds in four directions, release stop, return to center and emergency stop. Protocol terminal success remains separate from observed physical direction and motion.
