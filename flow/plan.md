...
目标：以已经通过用户真机验收的手动控制和四个固定动作为硬件基线，完成自定义舞蹈、默认舞蹈和左右/上下独立角度与速度设置的 Windows → EasyInput → 小智真实链路；同时保留 T16 白名单应用/Codex 简报和 T17 双来源记忆的独立验收。两套 T15D V2 固件候选已经构建并审计，但必须由用户逐板明确授权后才可 app-only 烧录。

### Current execution point

> **主 Agent 总控快照（2026-09-03，以下内容优先于本节后面的历史分支记录）**
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制和 T15 四个固定动作已完成用户真机验收；用户确认动作可用但希望上下幅度更明显、速度可调，并要求保存的自定义舞蹈可成为“跳舞”的真实内容。实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 已冻结并实现 T15D V2：Host HID 继续使用 `0x1A/0x1B`，Link 改用新增 `0x26/0x27`；设置页直接保存左右 `4..40°`、上下 `4..20°` 和两轴各 `20..100°/s`，小智依据原始云台限位和 Stage 2 脉宽包络再次限幅。内置默认舞蹈可见、可复制；只有明确激活的已保存舞蹈才会替换快速动作和语音“跳舞”。Windows 包和两套 app-only 候选均已构建并审计；本轮没有接触硬件，下一道门是分别授权烧录 EasyInput 与小智，再做实体角度、速度和编舞真机验收。T16、T17 保持代码完成、用户验收待办。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` | Fixed actions physically accepted; T15D V2 Windows `375/375`, native/package; EasyInput Host `15/15` + ESP-IDF v5.5.5; Xiaozhi Host `16/16` + ESP-IDF v5.5.3 | `T15_PRESET_HIL_ACCEPTED / T15D_V2_THREE_END_CODE_BUILD_CONFIRMED / DUAL_FIRMWARE_FLASH_AUTHORIZATION_PENDING` |
| T15D physical choreography | integrated at `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` | Frozen Host `0x1A/0x1B` V2 and Link `0x26/0x27`; 2–8 beats, three expressions, visible built-in dance, explicit custom activation, Yaw `4..40°`, Pitch `4..20°`, independent `20..100°/s`; both exact app-only candidates audited | `V2_WIRE_FROZEN / WINDOWS_AND_DUAL_FIRMWARE_BUILD_CONFIRMED / HIL_PENDING` |
