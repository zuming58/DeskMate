...
- T11D Windows companion playback/stop hardening failed its user-present gate. The exact package was running. Sanitized evidence and characterization tests show that the fixed three-second renderer backlog silently stops all scheduled nodes (`queueDrops=3`) and that whole-`runtime` stale snapshots can roll a completed main-process idle back to renderer stopping. Reflected ASR was not observed (`ignoredAsrDuringPlayback=0`). Status is `T11D_HIL_FAILED / AUDIT_COMPLETE / PRODUCTION_REPAIR_NOT_STARTED`; do not begin UI polish or another feature slice. The next Windows package must implement the evidence-backed bounded queue/explicit-failure, atomic ordered runtime reducer, awaited stop reconciliation and sanitized lifecycle/build identity design in [`t11d-companion-stop-drain-hil-failure-audit-2026-09-01.md`](../docs/reviews/t11d-companion-stop-drain-hil-failure-audit-2026-09-01.md).
