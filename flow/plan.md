...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已确认 T21 三段式实时对话、语音动作和真人插话时机可用，但第二轮真人验收发现安静时也会频繁误打断。内容安全诊断和源码复核已定位为“历史累计识别文本被当成本轮发言”与 Qwen VAD `0.0` 过敏设置叠加；当前包已改为只看当前 utterance、采用 `0.2` 平衡阈值，并要求服务端 speech item 加稳定文本证据。自动回归和 Windows 包已通过，真实房间噪声、扬声器回声、普通真人插话与后台真人唤醒仍待用户验收。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21B three-stage companion, evidence-gated barge-in and local wake repair | `codex/t21-three-stage-streaming-companion@08e7f4b9c8e8f1973be5e5642305cffe68c27926` | User confirmed realtime conversation, voice motion and that real speech can interrupt; false-trigger HIL rejected the prior candidate; cumulative-history contamination and noise-sensitive VAD were repaired with current-item/server-speech/stability evidence; Desktop `445/445`, package and InputBridge passed | `BASE_DIALOGUE_AND_BARGE_TIMING_HIL_ACCEPTED / FALSE_TRIGGER_HIL_REJECTED / EVIDENCE_GATED_REPAIR_CODE_BUILD_CONFIRMED / SYNTHETIC_WAKE_SMOKE_PASSED / USER_NOISE_ECHO_SPEECH_AND_WAKE_RETEST_PENDING / FIRMWARE_UNCHANGED` |
...
10. **T16A desktop actions and Codex live briefing — CODE/BUILD COMPLETE; TWO-REAL-TASK HIL PENDING.** Direct launch remains restricted to explicitly voice-enabled registered applications. Existing Codex lifecycle Hooks now emit an opaque per-task identity into one bounded store; a separate read-only App Server catalog contributes only visible task titles and is never treated as live status. Aggregate questions report the active count and every bounded task line; unique names select one task; similar names request clarification. Trusted turns replace the current Doubao provider generation before speaking, so stale free-chat text/audio cannot race the deterministic answer. No model-authored percentage, ETA or progress fraction is accepted. Ordinary thinking/working/checking updates update state silently and never auto-open the companion; only waiting, completed and explicit error may proactively notify when the user's switch is on. Contract: [`t16a-codex-live-task-monitor-v1.md`](../docs/contracts/t16a-codex-live-task-monitor-v1.md).
...
14. **T20 companion entry/timing closure — WINDOWS CODE/BUILD COMPLETE; USER HIL PENDING.** Windows `System.Speech` with the installed `zh-CN` recognizer is the explicitly enabled local background wake engine: it receives only bounded exact phrases, keeps the capsule hidden, emits only ready/wake events, yields the microphone during dictation/realtime companion/mic test and resumes afterward. The foreground session freezes a four-second provider endpoint and ten-second listening-only idle exit, then hides the capsule and resumes wake. Duplicate wake callbacks are debounced. Every semantic action has a local generated sound fallback; dance still prefers the enabled user-selected local track. This machine passed a real two-second listen/release smoke; phrase recognition and perceived timing remain user-observed gates. Contract: [`t20-background-wake-session-v1.md`](../docs/contracts/t20-background-wake-session-v1.md).
15. **T21 three-stage streaming companion — BASE DIALOGUE AND BARGE TIMING USER-ACCEPTED; T21B FALSE-TRIGGER RETEST PENDING.** Bailian streaming ASR, one stateful DeskMate CompanionModel and sentence-streamed Doubao TTS are active; the user confirmed ordinary realtime dialogue, voice motion and that real speech can interrupt. T21B now keeps ASR input open for the whole audible playback window but evaluates only the current provider utterance. Qwen server VAD uses balanced threshold `0.2`; explicit interrupt phrases need a matching server speech start, while ordinary speech needs stable progressive partials or a duration-backed final. Cumulative old transcript, fillers, one unstable hypothesis and close assistant echo cannot cancel playback. Background wake uses bounded finite PCM windows and passes the generated 16 kHz phrase smoke; human wake, room-noise immunity, speaker-echo immunity and ordinary-sentence barge-in remain the user gate. Contracts: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md), [`t21b-recognized-barge-in-and-local-wake-repair-v1.md`](../docs/contracts/t21b-recognized-barge-in-and-local-wake-repair-v1.md).
