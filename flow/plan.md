...
10. **T16A desktop actions and Codex live briefing — CODE/BUILD COMPLETE; TWO-REAL-TASK HIL PENDING.** Direct launch remains restricted to explicitly voice-enabled registered applications. Existing Codex lifecycle Hooks now emit an opaque per-task identity into one bounded store; a separate read-only App Server catalog contributes only visible task titles and is never treated as live status. Aggregate questions report the active count and every bounded task line; unique names select one task; similar names request clarification. Trusted turns replace the current Doubao provider generation before speaking, so stale free-chat text/audio cannot race the deterministic answer. No model-authored percentage, ETA or progress fraction is accepted. The optional explicit reporter remains available for richer 80-character milestones and explicit failure, because the official lifecycle Hook has no general root-turn failure event. Contract: [`t16a-codex-live-task-monitor-v1.md`](../docs/contracts/t16a-codex-live-task-monitor-v1.md).
