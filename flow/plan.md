...
- T08 已按用户确认开启并行固件分工：EasyInput 窗口从 `origin/main@3e2a046f49260ead422da4c295c3321de13dca5d` 在 `codex/easyinput-t08-link-controller` 单点拥有共享 DeskMate Link v1 合同和总控实现；小智窗口从只含分工文档的协调提交 `90585a6d23a448ad12433c112d451ddea8c737f5` 建立 `codex/xiaozhi-t08-link-endpoint`，只开发小智端。合同仍为 `NOT_FROZEN`，小智窗口在取得准确合同提交前只允许完成 UART/控制台证据核对、正式工程骨架、transport abstraction 和 Host test，不得猜 framing 或接线。任务见 [`T08-easyinput-link-controller.md`](tasks/T08-easyinput-link-controller.md) 与 [`T08-xiaozhi-link-endpoint.md`](tasks/T08-xiaozhi-link-endpoint.md)。
