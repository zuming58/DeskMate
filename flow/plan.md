...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。T21 从最新 `codex/t18-software-closure` 基线创建独立测试分支，已完成三段式 provider、合同、单元回归和 Windows 包；真实 ASR、CompanionModel、TTS 的网络时序、声音和两轮上下文仍等待用户人工验收。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21 three-stage companion experiment | `codex/t21-three-stage-streaming-companion@983d2e2ce9616f2d6431217044eb3d9062796130` | Frozen Windows-only contract; streaming ASR/model/TTS adapters; stable sentence streaming; cancellation and content-free metrics; Desktop `436/436`, package and InputBridge regression passed | `WINDOWS_CODE_BUILD_CONFIRMED / USER_REAL_VOICE_HIL_PENDING / FIRMWARE_UNCHANGED` |
...
15. **T21 three-stage streaming companion — WINDOWS CODE/BUILD COMPLETE; USER HIL PENDING.** The end-to-end dialogue provider is replaced in the active test branch by Bailian streaming ASR, one stateful DeskMate CompanionModel and a sentence-streamed Doubao TTS adapter. ASR partials remain display-only; ordinary final text reaches the model exactly once; stable visible sentences may start TTS before the full answer; stop/interrupt/reconnect revoke old work. The configured endpoint silence value remains independent from the ten-second foreground idle timer. Existing deterministic Codex/motion/application/media routes bypass the conversational model as compatibility tools until native structured tools receive a separate frozen slice. Contract: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md).
