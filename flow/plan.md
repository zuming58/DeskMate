...
## Current stage: T20 quiet background wake and fast foreground session

目标：以已经通过用户真机验收的动作链和 T19 Windows 应用/本地媒体能力为冻结基线，关闭实时陪伴入口的两层时序：一句话连续静音 4 秒后回答，回答后前台聆听空闲 10 秒即收起并切回隐藏的 Windows 本地唤醒；只有命中“小智，小智”才显示胶囊并打开豆包会话。未选择本地歌曲时，关注、点头、寻找和跳舞使用本地即时合成音效。随后继续 T16 Codex 简报和 T17 长期记忆人工验收。当前阶段不修改 Host HID、DeskMate Link 或两套动作固件。

### Current execution point

> **主 Agent 总控快照（2026-09-04，以下内容优先于本节后面的历史分支记录）**
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。当前工作树 `codex/t18-software-closure` 继续承接 Windows 软件修复；T20 把本机已通过进程 smoke 的 Windows 本地唤醒变成隐藏待机入口，并修正 4 秒句末判定、10 秒前台空闲收起和无歌曲时的内置动作音效。产品范围已收缩为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。
...
15. **EasyInput speaker downlink — CANCELLED.** Historical local-speaker code/build evidence remains auditable, but it is not an active delivery gate and must not be flashed or expanded without a new explicit product decision. T19 music uses the Windows computer speaker.
