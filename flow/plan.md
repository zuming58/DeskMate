...
- T11C companion half-duplex echo guard is implemented on `codex/t11c-companion-layout-echo-guard`. User-present T11B evidence already accepts the real Doubao/computer-microphone/computer-speaker main chain, while natural automatic barge-in remains rejected. The new default pauses microphone upload and ignores ASR during actual playback, maps playback to `working`, resumes `listening` on `tts.end` or manual interruption, requests browser AEC/noise/AGC/mono, and makes the realtime face the first Companion visual. Contract: [`T11C_COMPANION_HALF_DUPLEX_ECHO_GUARD_V1_FROZEN`](../docs/contracts/t11c-companion-half-duplex-echo-guard-v1.md).
