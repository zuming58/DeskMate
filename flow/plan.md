...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已确认 T21 三段式实时对话和语音动作可用；当前新门禁是 T21B 真人语音打断与后台唤醒。实现已从“播报时切断麦克风”升级为有意义 ASR 文本才能打断，并修复了旧 Windows 唤醒进程假就绪但不消费 PCM 的根因；自动回归、Windows 包和本机合成唤醒词烟测已通过，真实房间噪声/扬声器回声/真人唤醒仍待用户验收。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21B three-stage companion, recognized barge-in and local wake repair | `codex/t21-three-stage-streaming-companion@be9739b86053d0d9a84776c53981caf70246b0ce` | User confirmed realtime conversation and voice motion; recognized-text-only barge-in; repaired finite-memory local wake pipeline; Desktop `441/441`, package, InputBridge and generated 16 kHz wake phrase smoke passed | `BASE_DIALOGUE_HIL_ACCEPTED / T21B_WINDOWS_CODE_BUILD_CONFIRMED / SYNTHETIC_WAKE_SMOKE_PASSED / USER_BARGE_AND_WAKE_HIL_PENDING / FIRMWARE_UNCHANGED` |
