...
>
> T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收并保持冻结。用户已确认 T21 三段式实时对话和语音动作可用；当前新门禁是 T21B 真人语音打断与后台唤醒。第一轮打断候选被真人验收否决后，已确认问题不是旧包，而是豆包停止发送 PCM 后电脑仍在播放缓存尾音，原实现却过早关闭麦克风上行并清空回声上下文；修复版把打断窗口延长到本地扬声器真正播放完毕。自动回归、Windows 包和本机合成唤醒词烟测已通过，真实房间噪声/扬声器回声/真人唤醒仍待用户验收。产品范围继续限定为 Windows 软件、现有 EasyInput/小智已验收能力和 Codex 单一 Agent；硬件扩展、人脸跟随、人物/声纹、其他 Agent 与新音频下行均取消。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T21/T21B three-stage companion, recognized barge-in and local wake repair | `codex/t21-three-stage-streaming-companion@5fdb58a2673f4a1c8ec25b330b6353175b23c137` | User confirmed realtime conversation and voice motion; first barge HIL rejection traced to premature provider terminal vs local speaker drain; repaired full-audible-window interruption and finite-memory local wake pipeline; Desktop `443/443`, package, InputBridge and generated 16 kHz wake phrase smoke passed | `BASE_DIALOGUE_HIL_ACCEPTED / FIRST_BARGE_HIL_REJECTED / PLAYBACK_TAIL_REPAIR_CODE_BUILD_CONFIRMED / SYNTHETIC_WAKE_SMOKE_PASSED / USER_BARGE_AND_WAKE_RETEST_PENDING / FIRMWARE_UNCHANGED` |
...
15. **T21 three-stage streaming companion — BASE DIALOGUE USER-ACCEPTED; T21B HIL PENDING.** Bailian streaming ASR, one stateful DeskMate CompanionModel and sentence-streamed Doubao TTS are active; the user confirmed ordinary realtime dialogue and voice motion work. T21B keeps ASR input open only while this provider speaks, rejects fillers and assistant-text echo, and cancels the provider plus local playback as soon as a meaningful partial is recognized; its final opens exactly one replacement turn. Background wake now recognizes bounded finite PCM windows in memory because the former endless stream wrapper consumed no real PCM despite reporting ready. The generated 16 kHz local phrase smoke wakes successfully; human wake, non-speech immunity and speaker-echo immunity remain the current user gate. Contracts: [`t21-three-stage-streaming-companion-v1.md`](../docs/contracts/t21-three-stage-streaming-companion-v1.md), [`t21b-recognized-barge-in-and-local-wake-repair-v1.md`](../docs/contracts/t21b-recognized-barge-in-and-local-wake-repair-v1.md).
