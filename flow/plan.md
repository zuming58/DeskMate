...
| --- | --- | --- | --- |
| T10D-D current integrated candidate | `codex/t10d-d-simplified-manual-control@329b654` | Pitch semantic repair merged; focused `15/15` and full Desktop `320/320`; exact fixed Windows package running; firmware unchanged | `PHYSICAL_OUTPUT_CONFIRMED / PITCH_FIX_RUNNING / HIL_RERUN_PENDING` |
...
8. **T10D-D user-present HIL — ROUTE/OUTPUT CONFIRMED / PITCH FIX PACKAGE RUNNING.** The Windows-only semantic patch is merged and the exact new package is running: left/right remain Yaw `-1/+1`, while up/down are now Pitch `-1/+1` for this assembled mechanism. Focused `15/15` and full Desktop `320/320` pass; frozen wire bytes, request-ID persistence, firmware, centers, limits and step size are unchanged, so no reflash is required. Rerun up/down, release, center and emergency-stop observations. If accepted, freeze manual control and proceed to bounded preset movement design (nod/turn/attention first; dance and automatic expression linkage later).
