...
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收。T15D 实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 冻结 Host `0x1A/0x1B` 与 Link `0x26/0x27`，提供 Yaw `4..40°`、Pitch `4..20°` 及两轴 `20..100°/s`；EasyInput `AC31B817...56097` 与小智 `61193549...8EF43` 两个精确 V2 app-only 镜像均已获授权写入并独立回读一致。Windows 原生 V1/V2 修复 `0bf131dacb73c0dd6c8d420b4620de1ae3ffe110` 已集成到 `42138d7aab8180941efc5ec387edeb78608a8635`，Desktop `378/378`、打包和包内原生自检通过。用户随后确认快速动作、自定义舞蹈、角度和速度全部正常；T15D 升级为冻结基线且无需再次烧录。T16 的豆包播报和连续聆听已通过用户观察，但自由聊天仍曾抢答自然任务问法。实现提交 `dc3d364ff47c402df2e1c0424d8ae7831124ed83` 现让每个最终识别句都先进入实时 Bridge，普通聊天立即放行，可信任务/应用/动作回答由 Bridge 独占，并明确区分五秒说完等待和六十秒整段会话聆听；下一步用真实报告任务完成人工语音验收，再继续 T16 应用/动作、T17、T15C 和 T12 回归。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T18 current software closure | `codex/t18-software-closure@dc3d364ff47c402df2e1c0424d8ae7831124ed83` | Every accepted realtime final now crosses the Bridge before free chat; natural trusted task queries, immediate ordinary pass-through, five-second endpointing and persisted proactive-brief control are implemented; Desktop `393/393`, unpacked build and packaged native self-test | `WINDOWS_CODE_BUILD_CONFIRMED / REALTIME_BRIDGE_RUNTIME_READY / USER_CONVERSATION_AND_REPORTER_HIL_PENDING / FIRMWARE_UNCHANGED` |
...
10. **T16 desktop actions and Codex briefing — REALTIME BRIDGE CODE/BUILD COMPLETE; USER HIL PENDING.** Direct launch is implemented only for explicitly voice-enabled registered applications; the opt-in content-bounded task brief reporter is merged. User HIL accepted provider-native Doubao voice and direct post-playback recognition, but rejected provider free-chat answers to natural task questions. Commit `dc3d364ff47c402df2e1c0424d8ae7831124ed83` now routes every accepted final utterance through the Bridge: trusted Codex status owns the exact spoken answer, ordinary conversation passes immediately to Doubao, and bounded model classification is reserved for ambiguous control-like utterances. A persisted proactive-announcement switch controls speech without disabling task storage/query. Conversation/reporter, two-task selection, real application launch and voice motion still require user-controlled acceptance.
