...
## Current stage: T15 Windows motion repair, T15D choreography UX revision, T16 acceptance and T17 dual-source memory
...
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T15/T16 已完成三端代码、构建和双板烧录，但首次实体预设验收发现 Windows 原生桥把 `0x18/0x19` 错选到 `FF00:0007`，而冻结描述符实际使用独立 `FF00:0009`；当前先完成无需重烧固件的 Windows 修复，再恢复实体预设验收。T15D 交互简化和 T17 双来源记忆可在同一软件窗口继续，只有本总控分支负责项目级状态、合并顺序和最终验收。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T15/T16 current integration | `codex/t15-t16-integration@79ed688` | Desktop `359/359` + Windows package/native self-test; EasyInput Host `14/14` + ESP-IDF v5.5.5 build + app-only flash/readback; Xiaozhi Host `14/14` + ESP-IDF v5.5.3 build + user-reported flash success; first preset HIL isolated a Windows-only collection-selector defect | `DUAL_BOARD_FLASH_COMPLETE / PRESET_HIL_BLOCKED_WINDOWS_FF00_0009_FIX_IN_PROGRESS` |
...
| T17 dual-source memory | existing SQLite memory base, Windows delivery in `DeskMate软件开发` from `79ed688` | Shared review-first engine; separate `companion` / `dictation` sources; scheduled daily summaries with next-start catch-up; managed knowledge-base projection | `WINDOWS_IMPLEMENTATION_IN_PROGRESS / USER_HIL_PENDING / NO_FIRMWARE_CHANGE` |
...
9. **T15 runtime preset motion — DUAL FLASH COMPLETE; WINDOWS COLLECTION FIX IN PROGRESS.** Additive HID `0x18/0x19` and DeskMate Link `0x22/0x23` are frozen and implemented. First HIL proved manual motion still works but preset Feature `0x18` fails before the board because the native bridge selects `FF00:0007` instead of descriptor collection `FF00:0009`. This is a Windows-only defect and does not require either firmware to be reflashed. After the fix, rerun read-only status and then attention → nod twice → search → dance twice → emergency stop/recovery. Voice and automatic motion remain disabled until that sequence passes.
...
11. **T15D custom choreography — WINDOWS EDITOR MERGED; DIRECT-CONTROL UX REVISION IN PROGRESS; WIRE NOT FROZEN.** The strict 2–8-beat store/compiler remains, but user testing rejected dropdown-per-cell editing and the duplicated large face. The active software revision uses direct three-state buttons per Yaw/Pitch cell, three visual expression buttons, blank-as-hold, compact beat preview and one action bar below the grid. Entity execution remains visibly disabled behind a fail-closed pending adapter. New Host/Link IDs and both firmware endpoints stay blocked on the current four-preset physical HIL. Design: [`t15d-choreography-editor-v1.md`](../docs/design/t15d-choreography-editor-v1.md).
12. **T17 dual-source memory — WINDOWS IMPLEMENTATION IN PROGRESS.** Extend the existing SQLite/review/index/projection pipeline instead of creating a second memory system. Companion conversation and successful voice-input history become distinct sources with separate daily summaries, candidate review and filters. A local daily schedule may catch up on next launch; only text enters the memory store, reviewed items enter long-term context, and managed Markdown is written only under the user-selected DeskMate knowledge-base subdirectory. This slice changes no firmware.
