...
目标：以已经通过用户真机验收的手动控制、固定动作、可调角度/速度和自定义舞蹈为冻结硬件基线，优先关闭 Windows 软件链：T16 白名单应用与可信 Codex 简报、T17 双来源长期记忆、T15C 自动情境动作和 T12 陪伴设置回归。当前阶段不修改 Host HID、DeskMate Link 或两套动作固件；只有新的诊断证据证明冻结合同失效时才重新开启固件切片。

### Current execution point

> **主 Agent 总控快照（2026-09-04，以下内容优先于本节后面的历史分支记录）**
>
> 三个开发窗口的 Flow、Git 分支和验收事实已经重新汇总。T10D-D 手动控制、T15 四个固定动作和 T15D V2 可调动作/自定义舞蹈均已完成用户真机验收。T15D 实现提交 `d6ffb595dd4ea20decdfe6f114c5ffe56838e83c` 冻结 Host `0x1A/0x1B` 与 Link `0x26/0x27`，提供 Yaw `4..40°`、Pitch `4..20°` 及两轴 `20..100°/s`；EasyInput `AC31B817...56097` 与小智 `61193549...8EF43` 两个精确 V2 app-only 镜像均已获授权写入并独立回读一致。Windows 原生 V1/V2 修复 `0bf131dacb73c0dd6c8d420b4620de1ae3ffe110` 已集成到 `42138d7aab8180941efc5ec387edeb78608a8635`，Desktop `378/378`、打包和包内原生自检通过。用户随后确认快速动作、自定义舞蹈、角度和速度全部正常；T15D 升级为冻结基线且无需再次烧录。T16 的豆包播报和连续聆听已通过用户观察，但自由聊天仍曾抢答自然任务问法。实现 `dc3d364` 让每个最终识别句先进入 Bridge；最新实现 `274026b8edb1ceef03fcbd7807cf1cb2d35c195e` 又补齐任务简报开启的 60 秒自然追问上下文，以及 Bridge/报告器的可见、可导出健康证据。当前唯一直接人工门是启动后重新提交当前任务报告，核对两项健康状态，并验证“那现在怎么样了”与普通聊天分别走可信回答和豆包放行。

#### Current source-of-truth map

| Track | Exact branch / HEAD | Accepted evidence | Current classification |
| --- | --- | --- | --- |
| T18 current software closure | `codex/t18-software-closure@274026b8edb1ceef03fcbd7807cf1cb2d35c195e` | Every accepted realtime final crosses the Bridge; a received report opens bounded natural follow-up context, Bridge/reporter health is observable, and ordinary chat remains immediate pass-through; Desktop `394/394`, unpacked build and packaged native self-test | `WINDOWS_CODE_BUILD_CONFIRMED / OBSERVABLE_REALTIME_BRIDGE_RUNTIME_READY / USER_CONVERSATION_HIL_PENDING / FIRMWARE_UNCHANGED` |
...
10. **T16 desktop actions and Codex briefing — OBSERVABLE REALTIME BRIDGE CODE/BUILD COMPLETE; USER HIL PENDING.** Direct launch is implemented only for explicitly voice-enabled registered applications; the opt-in content-bounded task brief reporter is merged. Commit `dc3d364` routes every accepted final through the Bridge. Commit `274026b8edb1ceef03fcbd7807cf1cb2d35c195e` adds bounded report-driven follow-up context and separate health evidence, after the latest HIL exposed that the test task had never actually been submitted. Trusted task answers suppress free chat; ordinary conversation passes immediately to Doubao. A persisted proactive-announcement switch controls speech without disabling task storage/query. App restart deliberately clears task snapshots, so active reporters must resend rather than allowing stale status. Conversation/reporter, two-task selection, real application launch and voice motion still require user-controlled acceptance.
...
12. **T17 dual-source memory — STANDALONE WINDOWS PAGE COMPLETE; USER HIL PENDING.** One SQLite/review/index/projection pipeline captures completed companion turns and successful real voice-input text as separate sources. Management is now a top-level left-navigation page immediately above diagnostics rather than an AI-companion subtab. The local model prompt is built in: raw successful text remains authoritative, summaries and candidates are source/day separated, sensitive/control/mock/failed material is excluded, only reviewed candidates enter long-term context and can be locally embedded. The default schedule remains 23:30 with next-start catch-up; configured managed Markdown projection remains automatic and retryable. This slice changes no firmware.
13. **T15C automatic contextual motion — WINDOWS CODE/BUILD COMPLETE; USER HIL PENDING.** The frozen T15/T15D semantic action path now has one persisted global switch defaulting off plus a separate optional idle switch. Start-companion attention, delayed-thinking search, confirmation nod, Codex-complete happy/nod and 90-second idle search obey `emergency/fault > recenter > manual > explicit voice > context > idle`; blocked automatic actions are skipped rather than queued or replayed. Desktop `385/385`, package, native self-test and both firmware Host suites pass; no firmware source, Flash or device state changed.
14. **T12 companion closure — USER HIL PENDING.** Regress persona versioning, provider pause, idle timeout and EasyInput call action. The saved wake phrase remains an explicitly unavailable future local setting until a separate offline wake engine is selected; it must not imply background microphone use.
15. **T11E-B EasyInput local-speaker HIL — DEFERRED SEPARATE USER-AUTHORIZED GATE.** First audit the exact app-only image and preserved 16 MiB layout, then verify only the bounded low-volume startup probe and microphone-priority arbitration. It does not prove realtime speaker downlink, which remains `NOT_FROZEN`.
