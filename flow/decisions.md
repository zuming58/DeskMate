...
## D028 · Companion owns the consolidated desktop-management entry

- 日期：2026-08-29
- 决策：DeskMate 左侧一级导航固定为工作台、语音输入、陪伴、历史记录、词库、按键配置六个高频入口；设备连接、AI 联动、表情/动作、环境感知、设置与诊断保留原页面与路由，但统一从陪伴页右侧管理区进入。
- 原因：高频工作流需要稳定、短而可扫视的导航；桌宠与设备相关低频能力持续增加时，平铺会挤压空间并模糊主次。集中入口保留完整能力，同时为后续记忆、提醒与陪伴状态提供一个明确归属。
- 视觉与硬件边界：品牌、陪伴主视觉和表情入口统一使用用户选定的青蓝大眼睛软件形象；当前固定小智参考的 128×64 SSD1306 为 1-bit 单色屏，实机仍按黑底白眼设计，软件青蓝不得作为彩屏已确认的证据。
- 实现门禁：陪伴页可以先提供明确标注的本地软件预览，但不得由此猜测性接入音频、记忆持久化、提醒调度、Host Action、DeskMate Link、OLED 或舵机。
