...
## D035 · Voice modes keep separate endpointing, microphone ownership and local gain

- 日期：2026-09-07
- 决策：普通语音输入继续采用用户明确的“按一次开始、再按一次停止并提交”，不增加静音自动结束。实时 AI 陪伴继续使用服务端语音端点检测和独立的前台空闲收起计时；后台本地唤醒使用 DeskMate 当前选择的电脑麦克风，经渲染进程转为有界 PCM16 后交给 Windows 本机识别器，不能再隐式切换到系统默认麦克风。
- 音量：豆包陪伴语音和 Codex 主动工作提醒分别保存本地增益，默认 `75%` 与 `30%`，只作用于 DeskMate WebAudio 播放，不改变 Windows 系统总音量；活动会话内的工作提醒结束后必须恢复陪伴音量。
- 原因：三种语音入口承担不同交互：普通输入需要可控的长口述，陪伴需要自然轮次，唤醒需要隐藏待机。混用静音阈值、麦克风或音量会导致误提交、假监听和过响提醒。
- 边界：唤醒音频不上传、不保存，诊断只保留计数和状态；任何 provider 原生热词、板载麦克风输入或 EasyInput 扬声器下行都不因本决策被宣称已实现。
