...
以下跨端任务已规划但仍受前序门禁阻挡，不代表合同已经冻结：

- [`T05-easyinput-config-nvs.md`](../../flow/tasks/T05-easyinput-config-nvs.md)：必须先提出并自审完整配置读取/事务保存合同，显式标为 `CONFIG_V1_FROZEN` 后才能实现。
- [`T06-easyinput-host-actions.md`](../../flow/tasks/T06-easyinput-host-actions.md)：必须在 T05 锁定后提出并自审 Host Action 合同，显式标为 `HOST_ACTION_V1_FROZEN` 后才能实现。
