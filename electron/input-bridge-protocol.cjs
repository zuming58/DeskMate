...
class InputTriggerFilter {
  ...
  accept(event) {
    ...
    if (["host-action", "fixed-text", "fixed-text-result", "config-write", "config-ack", "config-snapshot", "config-progress", "config-capabilities"].includes(event.type)) return { kind: event.type, event };
