...
function validateMemoryPolicy(value = {}, { allowRuntime = false } = {}) {
  const allowed = new Set(["version", "enabledSources", "schedule", "dailyTime", "hourlyEnabled", "audioRetentionDays", "rawRetentionDays", ...(allowRuntime ? ["lastResults"] : [])]);
  ...
  if (![1, 2, 3].includes(value.version) || enabledSources.some((source) => !MEMORY_SOURCES.includes(source))) throw new Error("memory-policy-sources-invalid");
  ...
  const audioRetentionDays = Math.max(1, Math.min(365, Number(value.audioRetentionDays) || 7));
  const rawRetentionDays = Math.max(1, Math.min(365, Number(value.rawRetentionDays) || 20));
  if (!Number.isInteger(audioRetentionDays) || !Number.isInteger(rawRetentionDays)) throw new Error("memory-policy-retention-invalid");
  ...
  return Object.freeze({ version: 3, enabledSources: Object.freeze(enabledSources), schedule, dailyTime, hourlyEnabled, audioRetentionDays, rawRetentionDays, lastResults });
}

function normalizeMemoryPolicy(value = {}) {
  try { return validateMemoryPolicy({ version: [1, 2, 3].includes(value.version) ? value.version : 1, enabledSources: value.enabledSources, schedule: value.schedule, dailyTime: value.dailyTime, hourlyEnabled: value.hourlyEnabled, audioRetentionDays: value.audioRetentionDays, rawRetentionDays: value.rawRetentionDays, lastResults: value.lastResults }, { allowRuntime: true }); }
