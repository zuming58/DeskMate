const { contextBridge, ipcRenderer } = require("electron");

contextBridge.exposeInMainWorld("desktopBridge", {
  localHistory: (value) => ipcRenderer.invoke("local-history:command", value),
  localBackup: (value) => ipcRenderer.invoke("local-backup:command", value),
  getRestoreHandover: () => ipcRenderer.invoke('local-backup:handover'),
  acknowledgeRestore: id => ipcRenderer.invoke('local-backup:acknowledge', id),
  getCapabilities: () => ipcRenderer.invoke("desktop:get-capabilities"),
  getXiaozhiHardwarePolicy: () => ipcRenderer.invoke("desktop:get-xiaozhi-hardware-policy"),
  setXiaozhiHardwarePolicy: (enabled) => ipcRenderer.invoke("desktop:set-xiaozhi-hardware-policy", { enabled: Boolean(enabled) }),
  refreshLinkDiagnostics: () => ipcRenderer.invoke("desktop:refresh-link-diagnostics"),
  getManualCalibrationStatus: () => ipcRenderer.invoke("desktop:get-manual-calibration-status"),
  queryManualCalibration: () => ipcRenderer.invoke("desktop:query-manual-calibration"),
  sendManualCalibrationCommand: (value) => ipcRenderer.invoke("desktop:send-manual-calibration-command", value),
  onManualCalibrationStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("manual-calibration-status", handler); return () => ipcRenderer.removeListener("manual-calibration-status", handler); },
  getManualControlStatus: () => ipcRenderer.invoke("desktop:get-manual-control-status"),
  startManualControl: (value) => ipcRenderer.invoke("desktop:start-manual-control", value),
  establishManualControlCenter: () => ipcRenderer.invoke("desktop:manual-control-establish-center"),
  pressManualControlDirection: (direction) => ipcRenderer.invoke("desktop:manual-control-press", direction),
  releaseManualControlDirection: (direction) => ipcRenderer.invoke("desktop:manual-control-release", direction),
  recenterManualControl: () => ipcRenderer.invoke("desktop:manual-control-recenter"),
  emergencyStopManualControl: () => ipcRenderer.invoke("desktop:manual-control-emergency-stop"),
  endManualControl: (reason) => ipcRenderer.invoke("desktop:end-manual-control", reason),
  onManualControlStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("manual-control-status", handler); return () => ipcRenderer.removeListener("manual-control-status", handler); },
  getMotionStatus: () => ipcRenderer.invoke("desktop:get-motion-status"),
  runPreset: (value) => ipcRenderer.invoke("desktop:run-motion-preset", value),
  stopAndCenter: (source = "UI") => ipcRenderer.invoke("desktop:stop-motion-and-center", source),
  emergencyStop: (source = "UI") => ipcRenderer.invoke("desktop:emergency-stop-motion", source),
  clearEmergencyStopAndCenter: (source = "UI") => ipcRenderer.invoke("desktop:clear-motion-emergency-stop-and-center", source),
  onMotionPresetStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("motion-preset-status", handler); return () => ipcRenderer.removeListener("motion-preset-status", handler); },
  listChoreographies: () => ipcRenderer.invoke("desktop:list-choreographies"),
  getChoreographyStatus: () => ipcRenderer.invoke("desktop:get-choreography-status"),
  saveChoreography: (value) => ipcRenderer.invoke("desktop:save-choreography", value),
  copyChoreography: (name) => ipcRenderer.invoke("desktop:copy-choreography", name),
  deleteChoreography: (name) => ipcRenderer.invoke("desktop:delete-choreography", name),
  runChoreography: (value) => ipcRenderer.invoke("desktop:run-choreography", value),
  setDefaultDance: (name) => ipcRenderer.invoke("desktop:set-default-dance", name),
  getMotionSettings: () => ipcRenderer.invoke("desktop:get-motion-settings"),
  setMotionSettings: (value) => ipcRenderer.invoke("desktop:set-motion-settings", value),
  getMotionAutomationPolicy: () => ipcRenderer.invoke("desktop:get-motion-automation-policy"),
  setMotionAutomationPolicy: (value) => ipcRenderer.invoke("desktop:set-motion-automation-policy", value),
  onMotionAutomationStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("motion-automation-status", handler); return () => ipcRenderer.removeListener("motion-automation-status", handler); },
  onChoreographyStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("choreography-status", handler); return () => ipcRenderer.removeListener("choreography-status", handler); },
  getNetworkSummary: () => ipcRenderer.invoke("desktop:get-network-summary"),
  getEasyInputAudioStatus: () => ipcRenderer.invoke("desktop:get-easyinput-audio-status"),
  openEasyInputAudioSetup: () => ipcRenderer.invoke("desktop:open-easyinput-audio-setup"),
  startEasyInputMicTest: () => ipcRenderer.invoke("desktop:start-easyinput-mic-test"),
  stopEasyInputMicTest: () => ipcRenderer.invoke("desktop:stop-easyinput-mic-test"),
  onEasyInputAudioEvent: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("easyinput-audio-event", handler); return () => ipcRenderer.removeListener("easyinput-audio-event", handler); },
  startEasyInputVoiceRecording: () => ipcRenderer.invoke("desktop:start-easyinput-voice-recording"),
  stopEasyInputVoiceRecording: () => ipcRenderer.invoke("desktop:stop-easyinput-voice-recording"),
  cancelEasyInputVoiceRecording: () => ipcRenderer.invoke("desktop:cancel-easyinput-voice-recording"),
  onEasyInputVoiceRecordingEvent: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("easyinput-voice-recording-event", handler); return () => ipcRenderer.removeListener("easyinput-voice-recording-event", handler); },
  registerShortcut: (shortcut) => ipcRenderer.invoke("desktop:register-shortcut", shortcut),
  setGlobalShortcutsEnabled: (enabled) => ipcRenderer.invoke("desktop:set-global-shortcuts-enabled", Boolean(enabled)),
  setShortcutCapture: (active) => ipcRenderer.invoke("desktop:set-shortcut-capture", Boolean(active)),
  listApplications: () => ipcRenderer.invoke("desktop:list-applications"),
  listRegisteredApplications: () => ipcRenderer.invoke("desktop:list-registered-applications"),
  registerApplication: (token) => ipcRenderer.invoke("desktop:register-application", token),
  chooseApplication: () => ipcRenderer.invoke("desktop:choose-application"),
  testApplication: (id) => ipcRenderer.invoke("desktop:test-application", id),
  getApplicationVoicePolicy: (id) => ipcRenderer.invoke("desktop:get-application-voice-policy", id),
  setApplicationVoiceEnabled: (id, enabled) => ipcRenderer.invoke("desktop:set-application-voice-enabled", { id, enabled }),
  getDanceMusicStatus: () => ipcRenderer.invoke("desktop:get-dance-music-status"),
  chooseDanceMusic: () => ipcRenderer.invoke("desktop:choose-dance-music"),
  setDanceMusicEnabled: (enabled) => ipcRenderer.invoke("desktop:set-dance-music-enabled", Boolean(enabled)),
  loadDanceMusic: () => ipcRenderer.invoke("desktop:load-dance-music"),
  previewDanceMusic: () => ipcRenderer.invoke("desktop:preview-dance-music"),
  stopDanceMusic: () => ipcRenderer.invoke("desktop:stop-dance-music"),
  sendDanceMusicPlaybackEvent: (value) => ipcRenderer.send("desktop:dance-music-playback-event", value),
  onDanceMusicCommand: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("dance-music-command", handler); return () => ipcRenderer.removeListener("dance-music-command", handler); },
  onDanceMusicStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("dance-music-status", handler); return () => ipcRenderer.removeListener("dance-music-status", handler); },
  getCodexTaskBriefStatus: () => ipcRenderer.invoke("desktop:get-codex-task-brief-status"),
  setCodexTaskBriefAnnouncements: (enabled) => ipcRenderer.invoke("desktop:set-codex-task-brief-announcements", Boolean(enabled)),
  onCodexTaskBriefStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("codex-task-brief-status", handler); return () => ipcRenderer.removeListener("codex-task-brief-status", handler); },
  onCodexTaskBriefAnnouncement: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("codex-task-brief-announcement", handler); return () => ipcRenderer.removeListener("codex-task-brief-announcement", handler); },
  readKeyboardConfig: () => ipcRenderer.invoke("desktop:read-keyboard-config"),
  promptWorkbench: value => ipcRenderer.invoke('prompts:command', value),
  getPromptSetupPatch: () => ipcRenderer.invoke('prompts:setup-patch'),
  exportPrompts: () => ipcRenderer.invoke('prompts:export'),
  importPrompts: revision => ipcRenderer.invoke('prompts:import', revision),
  onPromptWorkbenchState: listener => { const handler = (_event, value) => listener(value); ipcRenderer.on('prompt-workbench-state', handler); return () => ipcRenderer.removeListener('prompt-workbench-state', handler); },
  onPromptWheel: listener => { const handler = (_event, value) => listener(value); ipcRenderer.on('prompt-wheel', handler); return () => ipcRenderer.removeListener('prompt-wheel', handler); },
  getStyleStudioStatus: () => ipcRenderer.invoke("style-studio:get-status"),
  listStyleStudioMedia: () => ipcRenderer.invoke("style-studio:list"),
  importStyleStudioSource: (value) => ipcRenderer.invoke("style-studio:import", value),
  readStyleStudioMedia: (value) => ipcRenderer.invoke("style-studio:read", value),
  removeStyleStudioMedia: (value) => ipcRenderer.invoke("style-studio:remove", value),
  generateStyleStudioImage: (value) => ipcRenderer.invoke("style-studio:generate", value),
  cancelStyleStudioImage: (requestId) => ipcRenderer.invoke("style-studio:cancel", requestId),
  acquireStyleStudioInput: (token) => ipcRenderer.invoke("style-studio:acquire-input", token),
  releaseStyleStudioInput: (token) => ipcRenderer.invoke("style-studio:release-input", token),
  onStyleStudioInput: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("style-studio-input", handler); return () => ipcRenderer.removeListener("style-studio-input", handler); },
  previewKeyboardConfigPatch: (patch) => ipcRenderer.invoke("desktop:preview-keyboard-config-patch", patch),
  commitKeyboardConfig: (token) => ipcRenderer.invoke("desktop:commit-keyboard-config", token),
  setTriggerConfig: (value) => ipcRenderer.invoke("desktop:set-trigger-config", value),
  setVoiceRecording: (recording) => ipcRenderer.invoke("desktop:set-voice-recording", Boolean(recording)),
  setVoiceState: (value) => ipcRenderer.invoke("desktop:set-voice-state", value),
  setManualAgentState: (value) => ipcRenderer.invoke("desktop:set-manual-agent-state", value),
  setActiveAgentProvider: (provider) => ipcRenderer.invoke("desktop:set-active-agent-provider", provider),
  getAgentProviderStatus: (provider) => ipcRenderer.invoke("desktop:get-agent-provider-status", provider),
  getCodexAgentStatus: () => ipcRenderer.invoke("desktop:get-codex-agent-status"),
  writeClipboard: (text) => ipcRenderer.invoke("desktop:clipboard-write", text),
  pasteActiveWindow: (text) => ipcRenderer.invoke("desktop:paste-active-window", text),
  keyDiagnostic: (event) => ipcRenderer.invoke("desktop:key-diagnostic", event),
  getBailianStatus: () => ipcRenderer.invoke("bailian:get-status"),
  saveBailianCredentials: (value) => ipcRenderer.invoke("bailian:save-credentials", value),
  clearBailianCredentials: () => ipcRenderer.invoke("bailian:clear-credentials"),
  getImage2Status: () => ipcRenderer.invoke("image2:get-status"),
  saveImage2Credentials: (value) => ipcRenderer.invoke("image2:save-credentials", value),
  clearImage2Credentials: () => ipcRenderer.invoke("image2:clear-credentials"),
  getAiServiceStatus: () => ipcRenderer.invoke("ai-services:get-status"),
  saveTextModelService: (value) => ipcRenderer.invoke("ai-services:save-text", value),
  clearTextModelService: () => ipcRenderer.invoke("ai-services:clear-text"),
  saveRealtimeVoiceService: (value) => ipcRenderer.invoke("ai-services:save-realtime", value),
  clearRealtimeVoiceService: () => ipcRenderer.invoke("ai-services:clear-realtime"),
  startCompanionConversation: (value) => ipcRenderer.invoke("companion:start", value),
  stopCompanionConversation: () => ipcRenderer.invoke("companion:stop"),
  interruptCompanionConversation: () => ipcRenderer.invoke("companion:interrupt"),
  getCompanionConversationStatus: () => ipcRenderer.invoke("companion:get-status"),
  getCompanionPreferences: () => ipcRenderer.invoke("companion:get-preferences"),
  setCompanionPreferences: (value) => ipcRenderer.invoke("companion:set-preferences", value),
  getCompanionPersona: () => ipcRenderer.invoke("companion:get-persona"),
  setCompanionPersona: (value) => ipcRenderer.invoke("companion:set-persona", value),
  getCompanionIntent: () => ipcRenderer.invoke("companion:get-intent"),
  confirmCompanionIntent: (token) => ipcRenderer.invoke("companion:confirm-intent", token),
  rejectCompanionIntent: (token) => ipcRenderer.invoke("companion:reject-intent", token),
  setCompanionStartOptions: (value) => ipcRenderer.invoke("companion:set-start-options", value),
  testCompanionCallAction: () => ipcRenderer.invoke("companion:test-call-action"),
  onCompanionConversationEvent: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("companion-conversation-event", handler); return () => ipcRenderer.removeListener("companion-conversation-event", handler); },
  setCompanionComputerAudioReady: (ready) => ipcRenderer.invoke("companion:set-computer-audio-ready", Boolean(ready)),
  sendCompanionComputerAudioEvent: (value) => ipcRenderer.send("companion:computer-audio-event", value),
  onCompanionComputerAudioCommand: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("companion-computer-audio-command", handler); return () => ipcRenderer.removeListener("companion-computer-audio-command", handler); },
  getMemoryStatus: () => ipcRenderer.invoke("memory:get-status"),
  getMemoryCuration: () => ipcRenderer.invoke('memory:get-curation'),
  setMemoryCuration: value => ipcRenderer.invoke('memory:set-curation', value),
  runMemoryCuration: () => ipcRenderer.invoke('memory:run-curation'),
  resolveMemoryCuration: value => ipcRenderer.invoke('memory:resolve-curation', value),
  getWorkbenchOverview: () => ipcRenderer.invoke("workbench:get-overview"),
  listPersonalReminders: () => ipcRenderer.invoke("reminders:list"),
  createPersonalReminder: (value) => ipcRenderer.invoke("reminders:create", value),
  completePersonalReminder: (id) => ipcRenderer.invoke("reminders:complete", id),
  deletePersonalReminder: (id) => ipcRenderer.invoke("reminders:delete", id),
  snoozePersonalReminder: (value) => ipcRenderer.invoke("reminders:snooze", value),
  onPersonalRemindersChanged: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("personal-reminders-changed", handler); return () => ipcRenderer.removeListener("personal-reminders-changed", handler); },
  getMemoryPolicy: () => ipcRenderer.invoke("memory:get-policy"),
  setMemoryPolicy: (value) => ipcRenderer.invoke("memory:set-policy", value),
  getLocalRetentionStatus: () => ipcRenderer.invoke("retention:get-status"),
  previewLocalRetention: (value) => ipcRenderer.invoke("retention:preview", value),
  confirmLocalRetention: (value) => ipcRenderer.invoke("retention:confirm", value),
  runLocalRetentionNow: (value) => ipcRenderer.invoke("retention:run-now", value),
  acknowledgeLocalRetention: (value) => ipcRenderer.invoke("retention:acknowledge", value),
  onLocalRetentionCleanup: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("local-retention-browser-cleanup", handler); return () => ipcRenderer.removeListener("local-retention-browser-cleanup", handler); },
  commitDictationMemory: (value) => ipcRenderer.invoke("memory:commit-dictation", value),
  listMemories: (value) => ipcRenderer.invoke("memory:list", value),
  listMemoryTurns: (value) => ipcRenderer.invoke("memory:list-turns", value),
  setMemoryCandidateState: (value) => ipcRenderer.invoke("memory:set-candidate-state", value),
  getMemoryCandidateReview: () => ipcRenderer.invoke('memory:get-candidate-review'),
  organizeMemoryCandidates: value => ipcRenderer.invoke('memory:organize-candidates', value),
  reviewMemoryCandidateBatch: value => ipcRenderer.invoke('memory:review-candidate-batch', value),
  updateMemoryCandidate: (value) => ipcRenderer.invoke("memory:update-candidate", value),
  prepareMemoryForget: (value) => ipcRenderer.invoke("memory:prepare-forget", value),
  confirmMemoryForget: (value) => ipcRenderer.invoke("memory:confirm-forget", value),
  exportReviewedMemories: () => ipcRenderer.invoke("memory:export-reviewed"),
  getKnowledgeBaseStatus: () => ipcRenderer.invoke("memory:get-knowledge-base-status"),
  openKnowledgeBaseFolder: () => ipcRenderer.invoke("memory:open-knowledge-base"),
  chooseKnowledgeBaseLocation: () => ipcRenderer.invoke("memory:choose-knowledge-base"),
  generatePendingMemories: () => ipcRenderer.invoke("memory:generate-pending"),
  getMemoryJournalStatus: () => ipcRenderer.invoke("memory:get-journal-status"),
  closeMemoryWorkday: () => ipcRenderer.invoke("memory:close-workday"),
  retryMemoryJournals: () => ipcRenderer.invoke("memory:retry-journals"),
  syncKnowledgeOsMemory: () => ipcRenderer.invoke("memory:sync-knowledgeos"),
  getKnowledgeOsStatus: () => ipcRenderer.invoke("memory:get-knowledgeos-status"),
  setKnowledgeOsSettings: (value) => ipcRenderer.invoke("memory:set-knowledgeos-settings", value),
  chooseKnowledgeOsAdapter: () => ipcRenderer.invoke("memory:choose-knowledgeos-adapter"),
  testKnowledgeOsConnection: () => ipcRenderer.invoke("memory:test-knowledgeos"),
  rebuildMemoryIndex: () => ipcRenderer.invoke("memory:rebuild-index"),
  searchMemoryIndex: (value) => ipcRenderer.invoke("memory:search-index", value),
  syncKnowledgeBase: () => ipcRenderer.invoke("memory:sync-knowledge-base"),
  transcribeBailian: (value) => ipcRenderer.invoke("bailian:transcribe", value),
  cancelBailian: (requestId) => ipcRenderer.invoke("bailian:cancel", requestId),
  organizeBailian: (value) => ipcRenderer.invoke("bailian:organize", value),
  editSelectedText: (value) => ipcRenderer.invoke("bailian:edit-selected-text", value),
  cancelBailianOrganizer: (requestId) => ipcRenderer.invoke("bailian:cancel-organize", requestId),
  startBailianRealtime: () => ipcRenderer.invoke("bailian:realtime-start"),
  appendBailianRealtime: (value) => ipcRenderer.invoke("bailian:realtime-append", value),
  finishBailianRealtime: (sessionId) => ipcRenderer.invoke("bailian:realtime-finish", sessionId),
  cancelBailianRealtime: (sessionId) => ipcRenderer.invoke("bailian:realtime-cancel", sessionId),
  onBailianRealtimeEvent: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("bailian-realtime-event", handler); return () => ipcRenderer.removeListener("bailian-realtime-event", handler); },
  onVoiceToggle: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("voice-toggle", handler); return () => ipcRenderer.removeListener("voice-toggle", handler); },
  onVoiceEditError: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("voice-edit-error", handler); return () => ipcRenderer.removeListener("voice-edit-error", handler); },
  onVoiceCancel: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("voice-cancel", handler); return () => ipcRenderer.removeListener("voice-cancel", handler); },
  onKeyDiagnostic: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("key-diagnostic", handler); return () => ipcRenderer.removeListener("key-diagnostic", handler); },
  onInputBridgeStatus: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("input-bridge-status", handler); return () => ipcRenderer.removeListener("input-bridge-status", handler); },
  onHostActionResult: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("host-action-result", handler); return () => ipcRenderer.removeListener("host-action-result", handler); },
  onAgentProviderState: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("agent-provider-state", handler); return () => ipcRenderer.removeListener("agent-provider-state", handler); },
  onCodexAgentState: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("codex-agent-state", handler); return () => ipcRenderer.removeListener("codex-agent-state", handler); },
  onNavigate: (listener) => { const handler = (_event, payload) => listener(payload); ipcRenderer.on("desktop-navigate", handler); return () => ipcRenderer.removeListener("desktop-navigate", handler); },
});
