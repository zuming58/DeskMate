...
class PromptWorkbenchController {
  ...
  async copy(id = this.selected) {
    ...
    this.transient = false; this.changed(); this.hide();
  ...
  async command(value = {}) {
    ...
    if (value.type === 'editing') { this.editing = value.active === true; return { ok: true }; }
    if (value.type === 'leave') { this.editing = false; this.transient = false; return { ok: true }; }
    if (value.type === 'copy') {
      if (this.busy) return { ok: false, reason: '操作正在完成，请稍候' };
