...
function boundedInteger(value, minimum, maximum, name) {
  if (!Number.isInteger(value) || value < minimum || value > maximum) throw new Error(`choreography-${name}-invalid`);
  return value;
}
...
function encodeChoreographyFeatureReport(value = {}) {
  ...
  if (report[6] === 1) {
    ...
    report[16] = boundedInteger(value.yawAmplitudeDegrees ?? 20, 4, 40, "yaw-amplitude");
    report[17] = boundedInteger(value.pitchAmplitudeDegrees ?? 15, 4, 20, "pitch-amplitude");
    report[18] = boundedInteger(value.yawSpeedDegreesPerSecond ?? 80, 20, 100, "yaw-speed");
    report[19] = boundedInteger(value.pitchSpeedDegreesPerSecond ?? 80, 20, 100, "pitch-speed");
    let changed = false;
  ...
  report.writeUInt16LE(crc16(report.subarray(1, 44)), 44);
  return report;
}
...
function parseEndpoint(bytes) {
  if (bytes.length !== 24 || bytes[12] >= ENDPOINT_RESULTS.length || bytes[13] >= ENDPOINT_STATES.length || bytes[14] > 8 || (bytes[15] !== 0xff && bytes[15] > 7) || bytes[16] > 3 || bytes[17] > bytes[16] || bytes[18] > 4 || bytes[20] > 40 || bytes[21] > 20 || bytes[22] > 100 || bytes[23] > 100) throw new Error("choreography-endpoint-invalid");
  ...
  return Object.freeze({
    ...
    result: ENDPOINT_RESULTS[bytes[12]], state: ENDPOINT_STATES[bytes[13]], beatCount: bytes[14], currentBeat: bytes[15], repeat: bytes[16], completedRepeats: bytes[17], sourceCode: bytes[18], yawAmplitudeDegrees: bytes[20], pitchAmplitudeDegrees: bytes[21], yawSpeedDegreesPerSecond: bytes[22], pitchSpeedDegreesPerSecond: bytes[23], flags,
