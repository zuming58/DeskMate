...
test("forget confirmations are one-use, revision-bound and whole-store erasure is complete", () => withMemoryStore(({ store, now, advance }) => {
  ...
  assert.deepEqual(store.status(), { ready: true, storage: "sqlite-wal", curation: { enabled: false, consentAt: 0, nextRetryAt: 0, failures: 0, lastRunAt: 0, pending: 0, questions: 0, archived: 0, processed: 0, automatic: 0 }, candidateReview: { total: 0, unorganized: 0, modelPending: 0, review: 0, reference: 0, groups: 0 }, turns: 0, dailySummaries: 0, pendingCandidates: 0, longTermMemories: 0, embeddings: 0, unprocessedTurns: 0, unprocessedDays: 0, indexedChunks: 0, sourceCounts: { companion: { turns: 0, unprocessed: 0 }, dictation: { turns: 0, unprocessed: 0 } } });
...
test("knowledge-base location is encrypted and renderer status never exposes the full path", () => {
  ...
  try {
    ...
    assert.equal(settings.status().storage, "application-local");
    assert.equal(settings.loadRoot(), path.join(userDataPath, "knowledge-base"));
...
test("memory UI is a shared top-level page with built-in rules while raw storage and paths stay in Electron main", async () => {
  const [page, app, preload, main] = await Promise.all([
  ...
  for (const copy of ["导出摘要与长期记忆", "清空本地记忆库", "保存纠正", "永久删除", "知识库位置", "[[双向链接]]", "混合检索", "内置整理规则", "无需填写提示词", "原样保存在本地 SQLite", "本地向量＋关键词检索", "打开笔记文件夹"]) assert.match(memoryPage, new RegExp(copy.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")));
