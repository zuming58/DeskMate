...
test('T40 nine provenance-backed styles and bounded intensity', () => {
  assert.equal(STUDIO_STYLES.length, 9);
  for (const s of STUDIO_STYLES) assert(fs.existsSync(new URL(`../public/assets/style-studio/${s.id}.png`, import.meta.url)));
  assert.equal(wrapStudioIndex(-1), 8); assert.equal(wrapStudioIndex(10001), 2);
...
test('T37 orbital positions are unique, bounded and cycle', () => {
  ...
  assert.equal(new Set(positions.map(p=>`${p.x.toFixed(3)},${p.y.toFixed(3)}`)).size, 9);
  ...
  assert.deepEqual(studioOrbit(0,0), studioOrbit(0,9));
});
