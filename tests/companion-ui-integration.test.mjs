...
test("keeps shared memory in primary navigation and companion-only tools in AI Companion", async () => {
  ...
  assert.deepEqual(labels, ["工作台", "语音输入", "AI 陪伴", "风格映像", "历史记录", "词库", "提示词", "按键配置", "记忆管理", "设备与诊断"]);
...
test("companion UI is a real main-process session entry and does not create a renderer microphone workflow", async () => {
  ...
  assert.match(companion, /单次噪声、短语气词和扬声器回声不会触发/);
  ...
  assert.match(pages, /只显示画面，不代表小智已经动作/);
  assert.match(pages, /实体结果仍以现场观察为准/);
