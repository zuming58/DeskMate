...
| host_test files | Frozen task matrix; Maker host-test structure reviewed at fixed commit | Independent stderr/non-zero tests for vectors, ownership, lifetime, queues, exact USB descriptor/report parsing, default serialization bytes, event-ring overflow recovery, wheel coalescing/boundaries/no-replay, vendor fail-closed, diagnostics, and source boundaries. |
