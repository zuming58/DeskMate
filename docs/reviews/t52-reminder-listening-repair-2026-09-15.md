...
## Follow-up report: old T51 still running

The user supplied another redacted report generated at 2026-09-15 19:22:29 local time. It still identifies `t51-personal-reminders`; the observed seven DeskMate processes belong to `release-t51`, and its ASAR still matches the recorded T51 hash. Therefore this is not a real-voice failure of the T52 candidate, which has not yet been started.

The last shutdown is completed with reason `listening-idle-timeout`; saved and applied idle timeout are both 10000 ms. This proves idle shutdown rather than a reported crash, not that every human utterance was captured. Provider/transport/dialog errors and drain timeouts are zero; all 32 drains completed. The last pipeline includes eight residual playback-echo drops and four weak-barge rejects. These are event counts, not eight confirmed missing human sentences. The last model first delta is 1798 ms with one attempt and no remote recall, which does not explain an utterance that never reached accepted recognition. Lifecycle reconnect count alone cannot prove physical/network reconnects because the controller also increments it when later sessions connect.

The T52 exact package check and final-ASAR behavioral probe were rerun successfully without production startup or provider requests. No new implementation, timeout preference change, user-profile mutation, restart or hardware action was performed for this diagnosis. Next: with the user ready, restart to the verified T52 candidate and re-test immediate post-playback speech; inspect a T52 report if still missed. Extending the idle timeout alone would not repair missing recognition. Authorized KnowledgeOS Wiki lookup returned no matching evidence and no degraded index.
