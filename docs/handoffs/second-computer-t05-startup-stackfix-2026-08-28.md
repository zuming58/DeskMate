...
- Pre-commit firmware candidate: exact ESP-IDF v5.5.5, esp32s3, MINIMAL_BUILD, fixed 16 MB partitions; app 325264 bytes. ELF stack frames: app_main 224, ConfigNvsStore::load 112, save_config_transaction 96, config_owner_task 96, input_owner_task 432 bytes.
- Final clean-HEAD rebuild, app SHA-256 and exact app-only end address remain pending after commit.

## Safety and next step

The old image and old burn authorization are invalid. Do not burn the pre-commit candidate. After pushing the final clean HEAD, present the new full commit hash, app SHA-256, size and 0x010000..end range for a fresh explicit authorization. No port scan, device identification, Flash/NVS read or write, erase, flash, monitor or HIL has been performed.
