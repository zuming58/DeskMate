...
| `0x18` | Host → Board Feature | T15 固定动作请求，63-byte payload |
| `0x19` | Board → Host Input | T15 固定动作 accepted/terminal 响应，63-byte payload |
| `0x1A` | Host → Board Feature | T15D 有界编舞请求，63-byte payload |
| `0x1B` | Board → Host Input | T15D 编舞 accepted/terminal 响应，63-byte payload |
| `0x1C` | Host → Board Feature | T43 风格映像旋钮按压易失租约，63-byte payload |
...
`0x18/0x19` 与 `0x1A/0x1B` 位于 FF00:0009 runtime-control 顶层集合；冻结合同分别见 `contracts/deskmate-host/easyinput-motion-presets-v1.md` 与 `contracts/deskmate-host/easyinput-choreography-v1.md`。`0x1C` 复用该集合的 64-byte Windows transport，只持有 RAM-only 页面租约；完整布局、TTL、恢复边界和黄金向量见 [`STYLE_STUDIO_INPUT_LEASE_V1_FROZEN`](../../contracts/deskmate-host/easyinput-style-studio-lease-v1.md)。它不得持久化或改写键盘配置。
