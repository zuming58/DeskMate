...
- T11B user-present acceptance proved the real Doubao/computer-microphone/computer-speaker chain. T11C uses strict half-duplex for the computer speaker: `listening` uploads PCM, ASR final enters `thinking`, and real playback enters `speaking/working` while suppressing upload plus ASR. T11D keeps that guard through a bounded AudioSink drain after network `tts.end`; only the matching last played sample, timeout cleanup or explicit manual interruption returns to `listening`. Automatic acoustic barge-in remains outside the accepted boundary.
