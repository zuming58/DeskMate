...
| Realtime voice | configuration only; first target is Doubao WebSocket | future companion ASR/chat/TTS | Credentials encrypted, but no network connection until interruption and Bridge contracts are implemented |
...
The current slice implements steps 1 storage primitives, the summary/candidate schema, status/search listing and candidate review. Automatic companion ingestion, summary scheduling, embedding generation, reminders and `F:\wiki` synchronization are intentionally not enabled yet. The SQLite database is the authoritative local source; a future wiki adapter may export reviewed Markdown summaries but must not become an untracked second source of truth.
...
DeskMate adopts the two-plane architecture, not the project-specific persona, navigation logic or credentials. The realtime adapter and interruption arbitration remain a later implementation slice.
