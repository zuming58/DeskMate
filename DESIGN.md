...
提示词页固定在可用窗口高度内，左侧约 45% 为标题与简短说明的快选，右侧约 55% 为完整正文阅读。正文使用清晰对比与较大字号，两栏独立框内滚动；选词不带动顶部场景栏或外层页面。推荐第 3 键呼叫语音助手、第 4 键用于提示词、第 8 键粘贴；这些共用键一处修改、所有场景共用，不随场景切换。硬件更改仍需在按键配置预览并确认；本机未同步修改不得被重新读取的板上旧配置覆盖。
